    ...
    *{"row": "MY_ROW_CLASS", "col_trim": "", "row_trim": ""}*.
    ...
    def to_latex(
        ...
        **CSS Conversion**
        ...
        hrules = (
            get_option("styler.latex.hrules") if hrules is None else hrules
        )
        if hrules:
            obj.set_table_styles(
                [
    ...
    def clear(self) -> None:
        ...
        self_attrs = [
            a for a in self.__dict__ if not callable(a)
        ]  # maybe more attrs
    ...
    def set_sticky(
        ...
        if not isinstance(obj, pd.MultiIndex):
            ...
            levels_: list[int] = (
                refactor_levels(levels, obj) if levels else range_idx
            )
...
def _highlight_value(
    ...
    if isinstance(
        data, DataFrame
    ):  # min/max must be done twice to return scalar
...
def _bar(
    ...
    def css_calc(
        x, left: float, right: float, align: str, color: str | list | tuple
    ):
        """
        ...
        if align == "left":
            ...
            z_frac: float = (
                0.5  # location of zero based on the left-right range
            )
            if align == "zero":
    ...
    if align == "mid":
        if (
            left >= 0
        ):  # "mid" is documented to act as "left" if all values positive
            align, left = "left", 0 if vmin is None else vmin
        elif (
            right <= 0
        ):  # "mid" is documented to act as "right" if all values negative
