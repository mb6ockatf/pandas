        ...
        if len(ic):
            col_names = [
                (
                    ...
                    if (
                        (r[ic[0]] is not None)
                        and r[ic[0]] not in self.unnamed_cols
                    )
                    else None
                )
                for r in header
        ...
        for i, (arr, name) in enumerate(zip(index, names)):
            ...
            if self.index_names is not None:
                ...
                if isinstance(converters, dict):
                    index_converter = (
                        converters.get(self.index_names[i]) is not None
                    )
        ...
        if try_num_bool and is_object_dtype(values.dtype):
            ...
            try:
                ...
                if non_default_dtype_backend:
                    ...
                    if result_mask.all():
                        result = IntegerArray(
                            np.ones(result_mask.shape, dtype=np.int64),
                            result_mask,
                        )
    ...
    def _do_date_conversions(
        ...
        for colspec in self.parse_dates:
            ...
            if (
                ...
                and colspec in self.index_names
            ):
                continue
