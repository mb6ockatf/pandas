...
class ArrowParserWrapper(ParserBase):
    ...
    def _get_pyarrow_options(self) -> None:
        ...
        for pandas_name, pyarrow_name in mapping.items():
            if (
                pandas_name in self.kwds
                and self.kwds.get(pandas_name) is not None
            ):
        ...
        self.convert_options["strings_can_be_null"] = (
            "" in self.kwds["null_values"]
        )
    ...
    def _finalize_pandas_output(self, frame: DataFrame) -> DataFrame:
        ...
        if self.header is None:
            ...
            if len(self.names) != num_cols:
                ...
                columns_prefix = [
                    str(x) for x in range(num_cols - len(self.names))
                ]
                ...
                if self.dtype is not None:
                    key, new_dtype = (
                        ...
                        if self.dtype.get(item) is not None
                            ...
                            frame.columns[item],
                            self.dtype.get(frame.columns[item]),
                        )
                    )
