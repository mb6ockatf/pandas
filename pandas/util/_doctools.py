...
class TablePlotter:
    ...
    def _get_cells(self, left, right, vertical) -> tuple[int, int]:
        ...
        if vertical:
            ...
            vcells = max(
                sum(self._shape(df)[0] for df in left), self._shape(right)[0]
            )
            hcells = (
                max(self._shape(df)[1] for df in left) + self._shape(right)[1]
            )
        else:
            vcells = max(
                [self._shape(df)[0] for df in left] + [self._shape(right)[0]]
            )
            hcells = sum(
                [self._shape(df)[1] for df in left] + [self._shape(right)[1]]
            )
    ...
    def plot(
        ...
        if vertical:
            ...
            for i, (_left, _label) in enumerate(zip(left, labels)):
                ...
                self._make_table(
                    ax, _left, title=_label, height=1.0 / max_left_rows
                )
            # right
    ...
    def _insert_index(self, data):
        ...
        if col_nlevels > 1:
            ...
            values = [
                data.columns._get_level_values(i)._values
    ...
    def _make_table(
        self, ax, df, title: str, height: float | None = None
    ) -> None:
        if df is None:
