...
def pytest_collection_modifyitems(items, config) -> None:
    ...
    ignored_doctest_warnings = [
        ...
        (
            "NDFrame.clip",
            "Downcasting behavior in Series and DataFrame methods",
        ),
        ...
        (
            "to_pytimedelta",
            "The behavior of TimedeltaProperties.to_pytimedelta",
        ),
...
@pytest.fixture(
    params=[Index, Series, pd.array], ids=["index", "series", "array"]
)
...
@pytest.fixture(
    params=[Index, Series, DataFrame, pd.array], ids=lambda x: x.__name__
)
...
indices_dict = {
    ...
    "complex64": Index(
        ...
        + 1.0j * np.arange(10, dtype="complex64")
    ),
    "complex128": Index(
        np.arange(10, dtype="complex128")
        + 1.0j * np.arange(10, dtype="complex128")
    ),
...
if has_pyarrow:
    idx = Index(
        pd.array([f"pandas_{i}" for i in range(10)], dtype="string[pyarrow]")
    )
