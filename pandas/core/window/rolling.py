...
class BaseWindow(SelectionMixin):
    ...
    def _prep_values(self, values: ArrayLike) -> np.ndarray:
        ...
        try:
            ...
            raise TypeError(
                f"cannot handle this type -> {values.dtype}"
            ) from err
    ...
    def _apply_pairwise(
        ...
        pairwise: bool | None,
        func: Callable[
            [DataFrame | Series, DataFrame | Series], DataFrame | Series
        ],
        numeric_only: bool,
    ) -> DataFrame | Series:
        """
    ...
    def aggregate(self, func=None, *args, **kwargs):
        ...
        result = ResamplerWindowApply(
            self, func, args=args, kwargs=kwargs
        ).agg()
...
class BaseWindowGroupby(BaseWindow):
    ...
    def _apply_pairwise(
        ...
        result = super()._apply_pairwise(
            target, other, pairwise, func, numeric_only
        )
        # 1) Determine the levels + codes of the groupby levels
        if other is not None and not all(
            len(group) == len(other)
            for group in self._grouper.indices.values()
        ):
            # GH 42915
...
class Window(BaseWindow):
    ...
    **on**
    ...
    def std(self, ddof: int = 1, numeric_only: bool = False, **kwargs):
        return zsqrt(
            self.var(
                ddof=ddof, name="std", numeric_only=numeric_only, **kwargs
            )
        )
...
class Rolling(RollingAndExpandingMixin):
    ...
    def _validate(self) -> None:
        ...
        if (
            self.obj.empty
            or isinstance(
                self._on, (DatetimeIndex, TimedeltaIndex, PeriodIndex)
            )
            or (
                isinstance(self._on.dtype, ArrowDtype)
                and self._on.dtype.kind in "mM"
    ...
    def _validate_datetimelike_monotonic(self) -> None:
        ...
        if not (
            ...
            or self._on.is_monotonic_decreasing
        ):
...
class RollingGroupby(BaseWindowGroupby, Rolling):
    ...
    def _validate_datetimelike_monotonic(self) -> None:
        ...
        for group_indices in self._grouper.indices.values():
            ...
            if not (
                group_on.is_monotonic_increasing
                or group_on.is_monotonic_decreasing
            ):
