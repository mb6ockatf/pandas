...
def _get_next_label(label):
    ...
    if lib.is_np_dtype(dtype, "mM") or isinstance(dtype, DatetimeTZDtype):
        ...
        raise TypeError(
            f"cannot determine next label for type {type(label)!r}"
        )
...
@inherit_names(
    ["is_non_overlapping_monotonic", "closed"], IntervalArray, cache=True
)
@set_module("pandas")
class IntervalIndex(ExtensionIndex):
    ...
    def _get_indexer(
        ...
        if isinstance(target, IntervalIndex):
            ...
            is_object_dtype(target.dtype) or is_string_dtype(target.dtype)
        ):
    ...
    def get_indexer_non_unique(
        ...
        elif is_object_dtype(target.dtype) or not self._should_partial_index(
            target
        ):
