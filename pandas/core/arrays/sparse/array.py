...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    ...
    def _take_with_fill(self, indices, fill_value=None) -> np.ndarray:
        ...
        if self.sp_index.npoints == 0 and old_fill_indices.all():
            ...
            taken = np.full(
                sp_indexer.shape, fill_value=fill_value, dtype=_dtype
            )
    ...
    def searchsorted(
        ...
        if get_option("performance_warnings"):
            ...
            warnings.warn(
                msg, PerformanceWarning, stacklevel=find_stack_level()
            )
        v = np.asarray(v)
    ...
    def map(self, mapper, na_action: Literal["ignore"] | None = None) -> Self:
        ...
        if na_action is None or notna(fill_val):
            fill_val = (
                mapper.get(fill_val, fill_val) if is_map else mapper(fill_val)
            )
        ...
        return type(self)(
            sp_values, sparse_index=self.sp_index, fill_value=fill_val
        )
    ...
    def _where(self, mask, value):
        ...
        dtype = SparseDtype(
            naive_implementation.dtype, fill_value=self.fill_value
        )
    ...
    def _min_max(self, kind: Literal["min", "max"], skipna: bool) -> Scalar:
        ...
        """
        valid_vals = self._valid_sp_values
        has_nonnull_fill_vals = (
            not self._null_fill_value and self.sp_index.ngaps > 0
        )
...
def make_sparse_index(
    length: int, indices, kind: Literal["block"]
) -> BlockIndex: ...


@overload
def make_sparse_index(
    length: int, indices, kind: Literal["integer"]
) -> IntIndex: ...


def make_sparse_index(
    length: int, indices, kind: SparseIndexKind
) -> SparseIndex:
    index: SparseIndex
    if kind == "block":
