...
@overload
def asarray_tuplesafe(
    values: Iterable, dtype: NpDtype | None = ...
) -> ArrayLike: ...


def asarray_tuplesafe(
    values: Iterable, dtype: NpDtype | None = None
) -> ArrayLike:
...
def convert_to_list_like(
    ...
    """
    if isinstance(
        values, (list, np.ndarray, ABCIndex, ABCSeries, ABCExtensionArray)
    ):
        return values
...
def fill_missing_names(names: Sequence[Hashable | None]) -> list[Hashable]:
    ...
    """
    return [
        f"level_{i}" if name is None else name for i, name in enumerate(names)
    ]
