...
def maybe_box_datetimelike(
    value: Scalar, dtype: Dtype | None = None
) -> Scalar:
    """
...
def maybe_downcast_to_dtype(
    result: ArrayLike, dtype: str | np.dtype
) -> ArrayLike:
    """
...
def maybe_downcast_numeric(
    ...
    if dtype.kind in "biu":
        ...
        if np.allclose(
            new_result, result, equal_nan=True, rtol=0.0, atol=atol
        ):
            return new_result
...
def convert_dtypes(
    ...
    if (
        ...
        if convert_boolean:
            if input_array.dtype.kind == "b":
                ...
                isinstance(inferred_dtype, str) and inferred_dtype == "boolean"
            ):
                inferred_dtype = pandas_dtype_func("boolean")
    ...
    if dtype_backend == "pyarrow" and not isinstance(
        inferred_dtype, ArrowDtype
    ):
        ...
        if (
            ...
            if isinstance(
                inferred_dtype, PandasExtensionDtype
            ) and not isinstance(inferred_dtype, DatetimeTZDtype):
    ...
    elif dtype_backend == "numpy_nullable" and isinstance(
        inferred_dtype, ArrowDtype
    ):
        # GH 53648
...
def maybe_cast_to_integer_array(
    arr: list | np.ndarray, dtype: np.dtype
) -> np.ndarray:
    """
...
def can_hold_element(arr: ArrayLike, element: Any) -> bool:
    ...
    if not isinstance(dtype, np.dtype) or dtype.kind in "mM":
        if isinstance(
            dtype, (PeriodDtype, IntervalDtype, DatetimeTZDtype, np.dtype)
        ):
...
def np_can_hold_element(dtype: np.dtype, element: Any) -> Any:
    ...
    if dtype.kind in "iu":
        ...
        if tipo is not None:
            ...
            if not isinstance(tipo, np.dtype):
                ...
                arr = (
                    element._values
                    if isinstance(element, ABCSeries)
                    else element
                )
                if arr._hasna:
                    raise LossySetitemError
    ...
    if dtype.kind == "c":
        if (
            lib.is_integer(element)
            or lib.is_complex(element)
            or lib.is_float(element)
        ):
            if np.isnan(element):
...
def _dtype_can_hold_range(rng: range, dtype: np.dtype) -> bool:
    ...
    if not len(rng):
        return True
    return np_can_cast_scalar(rng.start, dtype) and np_can_cast_scalar(
        rng.stop, dtype
    )
