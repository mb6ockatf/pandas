...
class CategoricalDtype(PandasExtensionDtype, ExtensionDtype):
    ...
    def _hash_categories(self) -> int:
        ...
        if len(categories) and isinstance(categories[0], tuple):
            ...
            if (
                categories.dtype == "O"
                and len({type(x) for x in categories}) != 1
            ):
    ...
    def update_dtype(
        ...
        new_ordered = (
            dtype.ordered if dtype.ordered is not None else self.ordered
        )
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        non_init_cats = [
            isinstance(x, CategoricalDtype) and x.categories is None
            for x in dtypes
        ]
        ...
        subtypes = (
            x.subtype if isinstance(x, SparseDtype) else x for x in dtypes
        )
...
class DatetimeTZDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    def __init__(
        self, unit: str_type | DatetimeTZDtype = "ns", tz=None
    ) -> None:
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        if all(
            isinstance(t, DatetimeTZDtype) and t.tz == self.tz for t in dtypes
        ):
            np_dtype = np.max(
...
class PeriodDtype(PeriodDtypeBase, PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    def __from_arrow__(self, array: pa.Array | pa.ChunkedArray) -> PeriodArray:
        ...
        if not results:
            return PeriodArray(
                np.array([], dtype="int64"), dtype=self, copy=False
            )
...
class IntervalDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    def __init__(
        self, subtype=None, closed: IntervalClosedType | None = None
    ) -> None:
