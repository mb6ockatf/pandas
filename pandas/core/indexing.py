...
class _LocationIndexer(NDFrameIndexerBase):
    ...
    def _convert_tuple(self, key: tuple) -> tuple:
        ...
        keyidx = [
            self._convert_to_indexer(k, axis=i) for i, k in enumerate(key)
        ]
        return tuple(keyidx)
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _setitem_with_indexer(
        self, indexer, value, name: str = "iloc"
    ) -> None:
        """
    ...
    def _setitem_with_indexer_split_path(self, indexer, value, name: str):
        ...
        if (isinstance(value, ABCSeries) and name != "iloc") or isinstance(
            value, dict
        ):
    ...
    def _setitem_single_column(self, loc: int, value, plane_indexer) -> None:
        ...
        is_full_setter = com.is_null_slice(pi) or com.is_full_slice(
            pi, len(self.obj)
        )
    ...
    def _ensure_iterable_column_indexer(self, column_indexer):
        ...
        if is_integer(column_indexer):
            ...
            isinstance(column_indexer, np.ndarray)
        ...
        if isinstance(indexer, tuple):
            ...
            if all(is_sequence(_) or isinstance(_, slice) for _ in indexer):
                ...
                if len(indexer) > 1 and not multiindex_indexer:
                    ...
                    ser_values = (
                        np.tile(ser_values, len_indexer)
