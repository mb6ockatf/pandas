...
def _from_dataframe(
    df: DataFrameXchg, allow_copy: bool = True
) -> pd.DataFrame:
    """
...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    data = buffer_to_ndarray(
        data_buff, data_dtype, offset=0, length=data_buff.bufsize
    )
...
def parse_datetime_format_str(format_str, data) -> pd.Series | np.ndarray:
    ...
    if date_meta:
        ...
        if unit == "D":
            ...
            data = (data.astype(np.uint64) * (24 * 60 * 60)).astype(
                "datetime64[s]"
            )
        elif unit == "m":
...
def buffer_to_ndarray(
    ...
    *,
    ...
    if bit_width == 1:
        assert (
            length is not None
        ), "`length` must be specified for a bit-mask buffer."
