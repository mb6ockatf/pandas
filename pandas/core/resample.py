...
class Resampler(BaseGroupBy, PandasObject):
    ...
    def _from_selection(self) -> bool:
        ...
        return self._timegrouper is not None and (
            ...
            or self._timegrouper.level is not None
        )
    ...
    def _groupby_and_aggregate(self, how, *args, **kwargs):
        ...
        grouped = get_groupby(
            obj, by=None, grouper=grouper, group_keys=self.group_keys
        )
...
class TimeGrouper(Grouper):
    ...
    def _adjust_bin_edges(
        ...
        if self.freq.name in ("BME", "ME", "W") or self.freq.name.split("-")[
            ...
            if self.closed == "right":
                ...
                edges_dti = (
                    edges_dti
                    + Timedelta(days=1, unit=edges_dti.unit).as_unit(
                        edges_dti.unit
                    )
    ...
    def _get_period_bins(self, ax: PeriodIndex):
        ...
        if isinstance(self.freq, Tick):
            ...
            start_offset = Period(start, self.freq) - Period(
                p_start, self.freq
            )
...
def asfreq(
    ...
    if isinstance(obj.index, PeriodIndex):
        ...
        if isinstance(obj.index, DatetimeIndex):
            ...
            unit = obj.index.unit
        dti = date_range(
            obj.index.min(), obj.index.max(), freq=freq, unit=unit
        )
