...
class TestDatetimeConcat:
    ...
    @pytest.mark.parametrize(
        "item", [pd.NaT, Timestamp("20150101").as_unit("ns")]
    )
...
class TestTimezoneConcat:
    ...
    def test_concat_tz_series6(self, unit, unit2):
        ...
        second = DataFrame(
            [[datetime(2016, 1, 2)], [datetime(2016, 1, 3)]],
            dtype=f"M8[{unit2}]",
        )
        second[0] = second[0].dt.tz_localize("Europe/London")
...
def test_concat_multiindex_datetime_nat():
    ...
    expected = DataFrame(
        {"a": [1.0, np.nan], "b": 2},
        MultiIndex.from_tuples([(1, pd.NaT), (2, pd.NaT)]),
    )
    tm.assert_frame_equal(result, expected)
...
def test_concat_float_datetime64():
    ...
    expected = DataFrame(
        {"A": pd.array(["2000"], dtype="datetime64[ns]")}
    ).astype(object)
