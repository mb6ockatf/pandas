...
class TestGetDummies:
    ...
    def test_get_dummies_basic_types(self, sparse, dtype, using_infer_string):
        ...
        if sparse:
            dtype_name = (
                f"Sparse[{self.effective_dtype(dtype).name}, {fill_value}]"
            )
        else:
    ...
    def test_dataframe_dummies_string_dtype(self, df, any_string_dtype):
        ...
        if any_string_dtype == "string" and any_string_dtype.na_value is pd.NA:
            expected[["B_b", "B_c"]] = expected[["B_b", "B_c"]].astype(
                "boolean"
            )
        tm.assert_frame_equal(result, expected)
        ...
        if sparse:
            ...
            expected = pd.concat(
                [
                    Series([1, 2, 3], name="C"),
                    Series(
                        [True, False, True], name="bad_a", dtype="Sparse[bool]"
                    ),
                    Series(
                        [False, True, False],
                        name="bad_b",
                    ...
                    Series(
                        [True, True, False], name="bad_b", dtype="Sparse[bool]"
                    ),
                    Series(
                        [False, False, True],
                        name="bad_c",
    ...
    def test_get_dummies_basic_drop_first_NA(self, sparse):
        ...
        exp_na = DataFrame(
            {"b": [0, 1, 0], np.nan: [0, 0, 1]}, dtype=bool
        ).reindex(["b", np.nan], axis=1)
        if sparse:
            exp_na = exp_na.apply(SparseArray, fill_value=False)
        tm.assert_frame_equal(res_na, exp_na)
    ...
    def test_get_dummies_int_int(self):
        ...
        expected = DataFrame(
            [[1, 0], [0, 1], [1, 0]], columns=[1, 2], dtype=bool
        )
        tm.assert_frame_equal(result, expected)
