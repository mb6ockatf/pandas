...
class TestEval:
    ...
    def test_complex_cmp_ops(
        self, cmp1, cmp2, binop, lhs, rhs, engine, parser
    ):
    ...
    def test_frame_invert(self, engine, parser):
        ...
        lhs = DataFrame(
            {
                "b": ["a", 1, 2.0],
                "c": np.random.default_rng(2).standard_normal(3) > 0.5,
            }
        )
        if engine == "numexpr":
...
class TestTypeCasting:
    ...
    def test_binop_typecasting(
        ...
        df = DataFrame(
            np.random.default_rng(2).standard_normal((5, 3)), dtype=dtype
        )
...
class TestOperations:
    ...
    def test_simple_arith_ops(self, engine, parser):
        ...
        for op in ops:
            ...
            if op in ("in", "not in"):
                ...
                y = self.eval(
                    ex2, local_dict={"x": x}, engine=engine, parser=parser
                )
                assert y == expec

                expec = _eval_single_bin(1, op, x + 1, engine)
                y = self.eval(
                    ex3, local_dict={"x": x}, engine=engine, parser=parser
                )
                assert y == expec
    ...
    def test_simple_in_ops(self, engine, parser):
        if parser != "python":
            ...
            res = pd.eval(
                "(3,) not in [(3,), 2]", engine=engine, parser=parser
            )
            assert not res

            res = pd.eval(
                "[(3,)] in [[(3,)], 2]", engine=engine, parser=parser
            )
            assert res
        else:
            ...
            with pytest.raises(NotImplementedError, match=msg):
                pd.eval(
                    "[(3,)] in (1, 2, [(3,)])", engine=engine, parser=parser
                )
            ...
            with pytest.raises(NotImplementedError, match=msg):
                pd.eval(
                    "[3] not in (1, 2, [[3]])", engine=engine, parser=parser
                )
...
class TestMath:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                np.complex128, np.complex128, marks=td.skip_if_windows
            ),
        ],
    )
...
def test_numexpr_option_incompatible_op():
    ...
    with pd.option_context("compute.use_numexpr", False):
        ...
        expected = DataFrame(
            {"A": [None, None], "B": [5, 6]}, index=range(4, 6)
        )
        tm.assert_frame_equal(result, expected)
