...
class TestPeriodRange:
    ...
    def test_construction_from_string(self, freq_offset, freq_period):
        ...
        result = period_range(
            start=start, end=end, freq=freq_period, name="foo"
        )
        tm.assert_index_equal(result, expected)

        result = period_range(
            start=start, periods=5, freq=freq_period, name="foo"
        )
        tm.assert_index_equal(result, expected)
        ...
        result = period_range(
            start=start, periods=0, freq=freq_period, name="foo"
        )
        tm.assert_index_equal(result, expected)
        ...
        result = period_range(
            start=end, end=start, freq=freq_period, name="foo"
        )
        tm.assert_index_equal(result, expected)
...
class TestPeriodRangeDisallowedFreqs:
    ...
    def test_uppercase_freq_deprecated_from_time_series(self, freq_depr):
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            period_range(
                "2020-01-01 00:00:00 00:00", periods=2, freq=freq_depr
            )
