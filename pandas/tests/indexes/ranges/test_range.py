...
class TestRangeIndex:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [RangeIndex(-4, 3, 2), RangeIndex(4, 7, 2)],
                RangeIndex(-4, 7, 2),
            ),
            ...
            (
                [RangeIndex(2), RangeIndex(2, 5), RangeIndex(5, 8, 4)],
                RangeIndex(0, 6),
            ),
            (
            ...
            (
                [RangeIndex(3, 1), Index(["a", None, 14])],
                Index(["a", None, 14]),
            ),
        ],
    )
...
def test_empty_argmin_argmax_raises(meth):
    with pytest.raises(
        ValueError, match=f"attempt to get {meth} of an empty sequence"
    ):
