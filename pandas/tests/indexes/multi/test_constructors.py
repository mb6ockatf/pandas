...
def test_from_product_invalid_input(invalid_input):
    msg = (
        r"Input must be a list / sequence of iterables|Input must be list-like"
    )
    with pytest.raises(TypeError, match=msg):
...
@pytest.mark.parametrize(
    "f", [lambda x: x, lambda x: Series(x), lambda x: x.values]
)
    ...
    df = pd.DataFrame(
        {
            ...
            "b": pd.Float64Dtype().__from_arrow__(
                pa.array([0.2, np.nan, None])
            ),
        }
    )
...
def test_raise_invalid_sortorder():
    ...
    with pytest.raises(
        ValueError, match=r".* sortorder 2 with lexsort_depth 1.*"
    ):
        MultiIndex(
            levels=levels,
    ...
    with pytest.raises(
        ValueError, match=r".* sortorder 1 with lexsort_depth 0.*"
    ):
        MultiIndex(
            levels=levels,
