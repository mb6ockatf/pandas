...
class TestDateRanges:
    ...
    def test_date_range_near_implementation_bound(self):
        ...
        with pytest.raises(
            OutOfBoundsDatetime, match="Cannot generate range with"
        ):
    ...
    @pytest.mark.parametrize(
        "s_ts, e_ts",
        [("2262-02-23", "1969-11-14"), ("1970-02-01", "1677-10-22")],
    )
    ...
    def test_compat_replace(self):
        ...
        result = date_range(
            Timestamp("1960-04-01 00:00:00"), periods=76, freq="QS-JAN"
        )
    ...
    def test_range_bug(self, unit):
        ...
        expected = DatetimeIndex(
            [start + i * offset for i in range(5)],
            dtype=f"M8[{unit}]",
            freq=offset,
        )
        tm.assert_index_equal(result, expected)
...
class TestGenRangeGeneration:
    ...
    def test_generate(self, freqstr, offset):
        rng1 = list(
            generate_range(START, END, periods=None, offset=offset, unit="ns")
        )
        rng2 = list(
            generate_range(START, END, periods=None, offset=freqstr, unit="ns")
        )
        assert rng1 == rng2
...
class TestDateRangeNonNano:
    ...
    def test_date_range_freq_matches_reso(self):
        ...
        dti = date_range(
            "2016-01-01", "2016-01-01 00:00:01", freq="ms", unit="ms"
        )
        ...
        dti = date_range(
            "2016-01-01", "2016-01-01 00:00:01", freq="us", unit="us"
        )
        rng = np.arange(
            1_451_606_400_000_000, 1_451_606_401_000_001, dtype=np.int64
        )
    ...
    def test_date_range_freq_lower_than_endpoints(self):
        ...
        rng = np.array(
            [start.as_unit("us")._value, end.as_unit("us")._value],
            dtype=np.int64,
        )
...
class TestDateRangeNonTickFreq:
    ...
    def test_date_range_custom_business_month_begin(self, unit):
        ...
        dti = date_range(
            start="20120101", end="20130101", freq=freq, unit=unit
        )
    ...
    def test_date_range_businesshour(self, unit):
        ...
        idx = DatetimeIndex(
            ["2014-07-04 16:00", "2014-07-07 09:00"],
            dtype=f"M8[{unit}]",
            freq="bh",
        )
        rng = date_range(
    ...
    def test_date_range_business_hour2(self, unit):
        ...
        idx2 = date_range(
            start="2014-07-04 15:00", periods=12, freq="bh", unit=unit
        )
        idx3 = date_range(
            end="2014-07-08 10:00", periods=12, freq="bh", unit=unit
        )
        expected = DatetimeIndex(
            [
                "2014-07-04 15:00",
        ...
        idx5 = date_range(
            start="2014-07-04 15:45", periods=12, freq="bh", unit=unit
        )
        idx6 = date_range(
            end="2014-07-08 10:45", periods=12, freq="bh", unit=unit
        )
    ...
    def test_date_range_business_hour_short(self, unit):
        # GH#49835
        idx4 = date_range(
            start="2014-07-01 10:00", freq="bh", periods=1, unit=unit
        )
    ...
    def test_date_range_negative_freq_year_end(self, unit):
        ...
        exp = DatetimeIndex(
            ["2011-12-31", "2009-12-31", "2007-12-31"],
            dtype=f"M8[{unit}]",
            freq="-2YE",
        )
        tm.assert_index_equal(rng, exp)
    ...
    def test_date_range_week_of_month(self, unit):
        ...
        result = date_range(
            start="20110101", periods=1, freq="WOM-1MON", unit=unit
        )
        expected = DatetimeIndex(
        ...
        result2 = date_range(
            start="20110101", periods=2, freq="WOM-1MON", unit=unit
        )
        expected2 = DatetimeIndex(
    ...
    def test_date_range_negative_freq_month_end(self, unit):
        ...
        exp = DatetimeIndex(
            ["2011-01-31", "2010-11-30", "2010-09-30"],
            dtype=f"M8[{unit}]",
            freq="-2ME",
        )
        tm.assert_index_equal(rng, exp)
