...
class TestInsert:
    @pytest.mark.parametrize(
        "null", [None, np.nan, np.datetime64("NaT"), NaT, NA]
    )
    ...
    def test_insert2(self, unit):
        idx = date_range(
            "1/1/2000", periods=3, freq="ME", name="idx", unit=unit
        )
    ...
    @pytest.mark.parametrize(
        "item",
        [0, np.int64(0), np.float64(0), np.array(0), np.timedelta64(456)],
    )
    def test_insert_mismatched_types_raises(self, tz_aware_fixture, item):
        ...
        if isinstance(item, np.ndarray):
            ...
            expected = Index(
                [dti[0], item] + list(dti[1:]), dtype=object, name=9
            )
