...
@pytest.fixture(
    params=["pytz/US/Eastern", gettz("US/Eastern"), ZoneInfo("US/Eastern")]
)
def tz(request):
...
class TestTZLocalize:
    def test_tz_localize_invalidates_freq(self):
        ...
        assert (
            result.inferred_freq is None
        )  # i.e. we are not _too_ strict here
    ...
    def test_dti_tz_localize_ambiguous_infer(self, tz):
        ...
        dr = date_range(
            datetime(2011, 11, 6, 0), periods=5, freq=offsets.Hour()
        )
    ...
    def test_dti_tz_localize_ambiguous_infer3(self, tz):
        ...
        dr = date_range(
            datetime(2011, 6, 1, 0), periods=10, freq=offsets.Hour()
        )
    ...
    def test_dti_tz_localize_ambiguous_times(self, tz):
        ...
        dr = date_range(
            datetime(2011, 3, 13, 1, 30), periods=3, freq=offsets.Hour()
        )
        ...
        # November 6, 2011, fall back, repeat 2 AM hour
        dr = date_range(
            datetime(2011, 11, 6, 1, 30), periods=3, freq=offsets.Hour()
        )
        with pytest.raises(ValueError, match="Cannot infer dst time"):
            dr.tz_localize(tz)
    ...
    def test_dti_tz_localize(self, prefix):
        ...
        dti = date_range(
            start="1/1/2005", end="1/1/2005 0:00:30.256", freq="ms"
        )
        ...
        dti = date_range(
            start="11/6/2011 1:59", end="11/6/2011 2:00", freq="ms"
        )
        with pytest.raises(ValueError, match="Cannot infer dst time"):
            dti.tz_localize(tzstr)

        dti = date_range(
            start="3/13/2011 1:59", end="3/13/2011 2:00", freq="ms"
        )
    ...
    def test_dti_tz_localize_nonexistent_shift(
        ...
        expected = (
            DatetimeIndex([Timestamp(end_ts)]).tz_localize(tz).as_unit(unit)
        )
        tm.assert_index_equal(result, expected)
