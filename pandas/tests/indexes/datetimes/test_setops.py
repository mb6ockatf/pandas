...
class TestCustomDatetimeIndex:
    ...
    def test_intersection_dst_transition(self, tz):
        ...
        index1 = date_range(
            "2021-10-28", periods=3, freq="D", tz="Europe/London"
        )
        index2 = date_range(
            "2021-10-30", periods=4, freq="D", tz="Europe/London"
        )
        result = index1.union(index2)
        expected = date_range(
            "2021-10-28", periods=6, freq="D", tz="Europe/London"
        )
        tm.assert_index_equal(result, expected)
...
def test_union_non_nano_rangelike():
    ...
    l2 = DatetimeIndex(
        ["2024-05-13"], dtype="datetime64[us]", name="Date", freq="D"
    )
    result = l1.union(l2)
    expected = DatetimeIndex(
