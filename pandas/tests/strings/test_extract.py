...
def test_extract_expand_kwarg(any_string_dtype):
    ...
    expected = DataFrame(
        [["BAD__", "BAD"], [np.nan, np.nan], [np.nan, np.nan]],
...
def test_extract_expand_capture_groups(any_string_dtype):
    ...
    expected = Series(
        ["A", "B", np.nan], name="letter", dtype=any_string_dtype
    )
    tm.assert_series_equal(result, expected)
...
def test_extract_dataframe_capture_groups_index(index, any_string_dtype):
    ...
    expected = DataFrame(
        ["1", "2", np.nan], index=index, dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, expected)
...
def test_extractall_single_group(any_string_dtype):
    s = Series(
        ["a3", "b3", "d4c2"], name="series_name", dtype=any_string_dtype
    )
    expected_index = MultiIndex.from_tuples(
...
def test_extractall_single_group_with_quantifier(any_string_dtype):
    ...
    # column.
    s = Series(
        ["ab3", "abc3", "d4cd2"], name="series_name", dtype=any_string_dtype
    )
    result = s.str.extractall(r"([a-z]+)")
    expected = DataFrame(
        ["ab", "abc", "d", "cd"],
...
def test_extractall_preserves_dtype():
    ...
    result = Series(
        ["abc", "ab"], dtype=ArrowDtype(pa.string())
    ).str.extractall("(ab)")
