...
def test_pad_fillchar(any_string_dtype):
    ...
    expected = Series(
        ["XXXXa", "XXXXb", np.nan, "XXXXc", np.nan, "eeeeee"],
        dtype=any_string_dtype,
    )
    tm.assert_series_equal(result, expected)
    ...
    expected = Series(
        ["aXXXX", "bXXXX", np.nan, "cXXXX", np.nan, "eeeeee"],
        dtype=any_string_dtype,
    )
    tm.assert_series_equal(result, expected)
    ...
    expected = Series(
        ["XXaXX", "XXbXX", np.nan, "XXcXX", np.nan, "eeeeee"],
        dtype=any_string_dtype,
    )
    tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize(
    "method_name", ["center", "ljust", "rjust", "zfill", "pad"]
)
...
def test_center_ljust_rjust_fillchar(any_string_dtype):
    ...
    tm.assert_series_equal(result, expected)
    expected = np.array(
        [v.center(5, "X") for v in np.array(s)], dtype=np.object_
    )
    ...
    tm.assert_series_equal(result, expected)
    expected = np.array(
        [v.ljust(5, "X") for v in np.array(s)], dtype=np.object_
    )
    tm.assert_numpy_array_equal(np.array(result, dtype=np.object_), expected)
    ...
    tm.assert_series_equal(result, expected)
    expected = np.array(
        [v.rjust(5, "X") for v in np.array(s)], dtype=np.object_
    )
    tm.assert_numpy_array_equal(np.array(result, dtype=np.object_), expected)
...
def test_zfill(any_string_dtype):
    ...
    expected = Series(
        ["001", "022", "aaa", "333", "45678"], dtype=any_string_dtype
    )
    tm.assert_series_equal(result, expected)
...
def test_wrap_unicode(any_string_dtype):
    ...
    s = Series(
        ["  pre  ", np.nan, "\xac\u20ac\U00008000 abadcafe"],
        dtype=any_string_dtype,
    )
    expected = Series(
