...
def test_iter_raises():
    ...
    ser = Series(["foo", "bar"])
    with pytest.raises(
        TypeError, match="'StringMethods' object is not iterable"
    ):
        iter(ser.str)
...
def test_count_mixed_object():
    ...
    expected = Series(
        [1, np.nan, 0, np.nan, np.nan, 0, np.nan, np.nan, np.nan]
    )
    tm.assert_series_equal(result, expected)
...
def test_empty_str_methods(any_string_dtype):
    ...
    tm.assert_frame_equal(
        empty_df.set_axis([], axis=1), empty.str.get_dummies()
    )
...
def test_len_mixed():
    ...
    result = ser.str.len()
    expected = Series(
        [3, np.nan, 13, np.nan, np.nan, 3, np.nan, np.nan, np.nan]
    )
    tm.assert_series_equal(result, expected)
...
def test_index(
    method, sub, start, end, index_or_series, any_string_dtype, expected
):
    obj = index_or_series(
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            5,
            None,
            ["foo", np.nan, "bar", np.nan, np.nan, None, np.nan, np.nan],
        ),
        (
            4,
            1,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            3,
            "z",
            ["shzrt", "a zit longer", "evznlongerthanthat", "z", np.nan],
        ),
        (
            2,
            2,
            "z",
            ["shzort", "a zbit longer", "evzenlongerthanthat", "z", np.nan],
        ),
        (
            2,
            1,
        ...
        (
            ...
            None,
            "z",
            ["shorz", "a bit longez", "evenlongerthanthaz", "z", np.nan],
        ),
        ...
        (
            ...
            8,
            "z",
            ["shortz", "a bit znger", "evenlozerthanthat", "z", np.nan],
        ),
        (
            -10,
            3,
            "z",
            ["zrt", "a zit longer", "evenlongzerthanthat", "z", np.nan],
        ),
    ],
)
...
def test_strip_lstrip_rstrip_mixed_object(method, exp):
    ...
    result = getattr(ser.str, method)()
    expected = Series(
        exp + [np.nan, np.nan, None, np.nan, np.nan], dtype=object
    )
    tm.assert_series_equal(result, expected)
...
def test_encode_decode(any_string_dtype):
    ser = Series(["a", "b", "a\xe4"], dtype=any_string_dtype).str.encode(
        "utf-8"
    )
