...
def test_split(any_string_dtype, method):
    values = Series(
        ["a_b_c", "c_d_e", np.nan, "f_g_h"], dtype=any_string_dtype
    )
...
def test_split_more_regex_split(any_string_dtype):
    # regex split
    values = Series(
        ["a,b_c", "c_d,e", np.nan, "f,g,h"], dtype=any_string_dtype
    )
...
def test_split_object_mixed(expand, method):
    mixed = Series(
        ["a_b_c", np.nan, "d_e_f", True, datetime.today(), None, 1, 2.0]
    )
...
def test_split_blank_string_with_non_empty(any_string_dtype):
    values = Series(
        ["a b c", "a b", "", " "], name="test", dtype=any_string_dtype
    )
    result = values.str.split(expand=True)
    exp = DataFrame(
        [
...
def test_split_no_pat_with_nonzero_n(
    data, pat, expected_val, any_string_dtype
):
    s = Series(data, dtype=any_string_dtype)
...
def test_split_to_dataframe_unequal_splits(any_string_dtype):
    s = Series(
        ["some_unequal_splits", "one_of_these_things_is_not"],
        dtype=any_string_dtype,
    )
    result = s.str.split("_", expand=True)
    exp = DataFrame(
...
def test_rsplit_to_dataframe_expand(any_string_dtype):
    ...
    exp = DataFrame(
        {0: ["some_equal", "with_no"], 1: ["splits", "nans"]},
        dtype=any_string_dtype,
    )
    tm.assert_frame_equal(result, exp)
...
def test_split_nan_expand(any_string_dtype):
    ...
    exp = DataFrame(
        [["foo", "bar", "baz"], [np.nan, np.nan, np.nan]],
        dtype=any_string_dtype,
    )
    tm.assert_frame_equal(result, exp)
...
def test_partition_series_none(any_string_dtype, method, exp):
    ...
    # None
    s = Series(
        ["a b c", "c d e", np.nan, "f g h", None], dtype=any_string_dtype
    )
...
def test_partition_to_dataframe(any_string_dtype, method, exp):
    ...
    s = Series(
        ["a_b_c", "c_d_e", np.nan, "f_g_h", None], dtype=any_string_dtype
    )
...
def test_get_mixed_object():
    ser = Series(
        ["a_b_c", np.nan, "c_d_e", True, datetime.today(), None, 1, 2.0]
    )
    result = ser.str.split("_").str.get(1)
    expected = Series(
