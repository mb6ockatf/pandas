...
_all_methods = [
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("__getitem__", np.array([True])),
    ),
    (
        pd.DataFrame,
        ({("A", "a"): [1]},),
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("eval", "A + 1", engine="python"),
    ),
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("select_dtypes", include="int"),
    ),
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("rename", columns={"A": "a"}),
    ),
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("rename", index=lambda x: x),
    ),
    ...
    (
        ...
        ({"A": [1], "B": [1]},),
        operator.methodcaller(
            "pivot_table", columns="A", aggfunc=["mean", "sum"]
        ),
    ),
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("squeeze", axis="columns"),
    ),
    ...
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("rename_axis", columns="a"),
    ),
    # Unary ops
    ...
    (
        pd.DataFrame,
        (frame_data,),
        operator.methodcaller("sample", n=2, replace=True),
    ),
    ...
    (
        pd.Series,
        ([1, 2],),
        operator.methodcaller("where", np.array([True, False])),
    ),
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("where", np.array([[True]])),
    ),
    (
        pd.Series,
        ([1, 2],),
        operator.methodcaller("mask", np.array([True, False])),
    ),
    (
        pd.DataFrame,
        frame_data,
        operator.methodcaller("mask", np.array([[True]])),
    ),
...
@pytest.mark.parametrize(
    "ndframe_method", _all_methods, ids=lambda x: idfn(x[-1])
)
...
def test_binops(request, args, annotate, all_binary_operators):
    ...
    if (
        not (isinstance(left, int) or isinstance(right, int))
        and annotate != "both"
    ):
