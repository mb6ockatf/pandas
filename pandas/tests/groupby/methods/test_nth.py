...
def test_first_last_with_na_object(method, nulls_fixture):
    ...
    groups = DataFrame(
        {"a": [1, 1, 2, 2], "b": [1, 2, 3, nulls_fixture]}
    ).groupby("a")
...
def test_first_strings_timestamps():
    ...
    expected = DataFrame(
        [["a", "c", "f"]],
        columns=Index(
            [Timestamp("2012-01-01"), Timestamp("2012-01-02"), "aaaa"]
        ),
...
def test_nth3():
    ...
    # from the vbench
    df = DataFrame(
        np.random.default_rng(2).integers(1, 10, (100, 2)), dtype="int64"
    )
    ser = df[1]
    gb = df[0]
...
def test_head_tail_dropna_true():
    # GH#45089
    df = DataFrame(
        [["a", "z"], ["b", np.nan], ["c", np.nan], ["c", np.nan]],
        columns=["X", "Y"],
    )
