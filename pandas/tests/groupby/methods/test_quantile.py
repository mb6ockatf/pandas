...
def test_quantile_array():
    ...
    expected = DataFrame(
        {"A": [0.25, 0.75, 2.25, 2.75], "B": [4.25, 4.75, 6.25, 6.75]},
        index=index,
    )
    tm.assert_frame_equal(result, expected)
...
def test_groupby_quantile_with_arraylike_q_and_int_columns(
    ...
    idx_codes = [[x for x in range(min(nrow, 4)) for _ in q]] * len(
        groupby
    ...
    expected_values = [
        [float(x)] * (ncol - len(groupby))
        for x in range(min(nrow, 4))
        for _ in q
    ]
...
def test_groupby_quantile_nullable_array(values, q):
    ...
    expected = pd.Series(
        true_quantiles * 2, index=idx, name="b", dtype="Float64"
    )
...
def test_groupby_quantile_NA_float(any_float_dtype):
    ...
    expected = pd.Series(
        [0.2], dtype=expected_dtype, index=exp_index, name="y"
    )
    tm.assert_series_equal(result, expected)
