...
def test_groupby_agg_dict_with_getitem():
    ...
    expected = DataFrame({"B": [3, 4]}, index=["A", "B"]).rename_axis(
        "A", axis=0
    )
...
def test_callable_result_dtype_frame(
    ...
    expected_index = (
        pd.RangeIndex(0, 1) if method == "transform" else agg_index
    )
    expected = DataFrame(
        {"c": [df["c"].iloc[0]]}, index=expected_index
    ).astype(result_dtype)
    if method == "apply":
...
def test_callable_result_dtype_series(keys, agg_index, input, dtype, method):
    ...
    expected = Series(
        [df["c"].iloc[0]], index=expected_index, name="c"
    ).astype(dtype)
    tm.assert_series_equal(result, expected)
...
def test_agg_relabel_multiindex_column(
    ...
    expected = DataFrame(
        {"col_1": agg_result1, "col_2": agg_result2, "col_3": agg_result3},
        index=idx,
    )
    tm.assert_frame_equal(result, expected)
...
def test_groupby_aggregate_empty_key_empty_return():
    ...
    expected = DataFrame(
        columns=MultiIndex(levels=[["b"], []], codes=[[], []])
    )
    tm.assert_frame_equal(result, expected)
...
def test_multiindex_custom_func(func):
    ...
    expected = DataFrame(
        expected_dict, index=np.array([0, 1]), columns=df.columns
    )
    tm.assert_frame_equal(result, expected)
