...
class TestGroupBy:
    ...
    def test_timegrouper_with_reg_groups(self):
        ...
        for df in [df_original, df_sorted]:
            ...
            df = df.reset_index()
            result = df.groupby(
                [Grouper(freq="1ME", key="Date"), "Buyer"]
            ...
            df = df.set_index("Date")
            result = df.groupby(
                [Grouper(freq="1ME", level="Date"), "Buyer"]
            ).sum(numeric_only=True)
            tm.assert_frame_equal(result, expected)
    ...
    def test_timegrouper_with_reg_groups_freq(self, freq):
        ...
        result1 = (
            df.sort_index()
            .groupby([Grouper(freq=freq), "user_id"])["whole_cost"]
            .sum()
        )
    ...
    def test_timegrouper_apply_return_type_series(self):
        ...
        result = df_dt.groupby(Grouper(freq="ME", key="date")).apply(
            sumfunc_series
        )
    ...
    def test_groupby_groups_datetimeindex2(self):
        ...
        expected = {
            Timestamp(date): DatetimeIndex([date], name="date")
            for date in dates
        }
    ...
    def test_groupby_max_datetime64(self):
        ...
        expected = (
            df.groupby("A")["A"].apply(lambda x: x.max()).astype("M8[s]")
        )
    ...
    def test_groupby_apply_timegrouper_with_nat_apply_squeeze(
        ...
        dti = Index(
            [Timestamp("2013-12-31")], dtype=df["Date"].dtype, name="Date"
        )
        expected = DataFrame(
            [[36, 6, 6, 10, 2]],
            index=dti,
