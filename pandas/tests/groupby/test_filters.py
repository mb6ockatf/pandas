...
def test_filter_against_workaround_dataframe():
    ...
    old_way = df[
        grouped.floats.transform(lambda x: x.mean() > N / 2).astype("bool")
    ]
    ...
    grouped = df.groupby(grouper)
    old_way = df[
        grouped.letters.transform(lambda x: len(x) < N / 2).astype("bool")
    ]
    ...
    old_way = df[
        grouped.ints.transform(lambda x: x.mean() > N / 2).astype("bool")
    ]
...
def test_filter_dropna_with_empty_groups():
    # GH 10780
    data = Series(
        np.random.default_rng(2).random(9), index=np.repeat([1, 2, 3], 3)
    )
