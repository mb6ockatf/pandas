...
def test_resample_as_freq_with_subperiod():
    ...
    expected = DataFrame(
        data=expected_data,
        index=timedelta_range("00:00:00", "00:10:00", freq="2min"),
    )
    tm.assert_frame_equal(result, expected)
...
def test_resample_single_period_timedelta():
    s = Series(
        list(range(5)), index=timedelta_range("1 day", freq="s", periods=5)
    )
...
def test_resample_categorical_data_with_timedeltaindex():
    ...
    exp_tdi = pd.TimedeltaIndex(
        np.array([0, 10], dtype="m8[s]"), freq="10s"
    ).as_unit("ns")
    expected = DataFrame(
...
def test_arrow_duration_resample():
    # GH 56371
    idx = pd.Index(
        timedelta_range("1 day", periods=5), dtype="duration[ns][pyarrow]"
    )
