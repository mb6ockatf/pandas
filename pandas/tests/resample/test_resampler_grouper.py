...
def test_groupby_resample_on_api_with_getitem():
    ...
    df = DataFrame(
        {
            "id": list("aabbb"),
            "date": date_range("1-1-2016", periods=5),
            "data": 1,
        }
    )
...
def test_apply_with_mutated_index():
    ...
    # A case for series
    expected = (
        df["col1"].groupby(pd.Grouper(freq="ME"), group_keys=False).apply(f)
    )
...
def test_apply_columns_multilevel():
    ...
    agg_dict = {
        col: (np.sum if col[3] == "one" else np.mean) for col in df.columns
    }
...
def test_resample_groupby_agg_listlike():
    ...
    df = DataFrame(
        {"class": ["beta"], "value": [69]}, index=Index([ts], name="date")
    )
...
def test_resample_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    idx = pd.MultiIndex.from_arrays(
        [
            ["A"] * 3 + ["B"] * 3,
            pd.to_datetime(
                ["2020-01-05", "2020-01-12", "2020-01-19"] * 2
            ).as_unit("ns"),
        ],
        names=["key", "date"],
    )
...
def test_resample_no_index(keys):
    ...
    result = df.groupby(keys).resample(rule=pd.to_timedelta("00:00:01")).mean()
    expected_columns = ["b"] if keys == ["a"] else []
    expected = DataFrame(columns=["a", "b", "date"]).set_index(
        keys, drop=False
    )
...
def test_resample_no_columns():
    ...
    result = (
        df.groupby([0, 0, 1]).resample(rule=pd.to_timedelta("06:00:00")).mean()
    )
...
def test_groupby_resample_on_index_with_list_of_keys_multi_columns():
    ...
    result = (
        df.groupby("group").resample("2D")[["first_val", "second_val"]].mean()
    )
