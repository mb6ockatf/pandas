...
def test_resample_how_callables(unit):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        ind = date_range(
            start="2014-01-01", periods=len(data), freq="d"
        ).as_unit(unit)
...
def test_resample_extra_index_point(unit):
    ...
    index = date_range(start="20150101", end="20150331", freq="B").as_unit(
        unit
    )
    df = DataFrame(
...
def test_nearest_upsample_with_limit(tz_aware_fixture, freq, rule, unit):
    # GH 33939
    rng = date_range(
        "1/1/2000", periods=3, freq=freq, tz=tz_aware_fixture
    ).as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), rng)
...
def test_resample_ohlc_result(unit):
    ...
    index = index.union(
        date_range("4-15-2000", "5-15-2000", freq="h").as_unit(unit)
    )
    s = Series(range(len(index)), index=index)
...
def test_resample_ohlc_result_odd_period(unit):
    ...
    expected = df.reindex(
        index=date_range(rng[0], rng[-1], freq="B").as_unit(unit)
    )
    tm.assert_frame_equal(result, expected)
...
def test_ohlc_5min(unit):
    ...
    rng = date_range(
        "1/1/2000 00:00:00", "1/1/2000 5:59:50", freq="10s"
    ).as_unit(unit)
...
def test_downsample_non_unique(unit):
    ...
    ts = Series(
        np.random.default_rng(2).standard_normal(len(rng2)), index=rng2
    )
...
@pytest.mark.parametrize(
    "freq", ["min", "5min", "15min", "30min", "4h", "12h"]
)
def test_resample_anchored_ticks(freq, unit):
    ...
    rng = date_range("1/1/2000 04:00:00", periods=86400, freq="s").as_unit(
        unit
    )
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
...
def test_resample_single_group(end, unit):
    ...
    tm.assert_series_equal(
        ts.resample("ME").sum(), ts.resample("ME").apply(mysum)
    )
...
def test_resample_offset(unit):
    ...
    exp_rng = date_range(
        "12/31/1999 23:57:00", "1/1/2000 01:57", freq="5min"
    ).as_unit(unit)
...
def test_resample_origin(kwargs, unit):
    # GH 31809
    rng = date_range(
        "2000-01-01 00:00:00", "2000-01-01 02:00", freq="s"
    ).as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
...
def test_resample_origin_prime_freq(unit):
    ...
    resampled = ts.resample(
        "17min", origin="start_day", offset="23h30min"
    ).mean()
    tm.assert_index_equal(resampled.index, exp_rng)
...
def test_resample_origin_epoch_with_tz_day_vs_24h(unit):
    ...
    tm.assert_series_equal(
        result_1.tz_localize(None), result_5.tz_localize(None)
    )
    tm.assert_series_equal(
        result_1.tz_localize(None), result_6.tz_localize(None)
    )
...
def test_resample_dst_midnight_last_nonexistent():
    ...
    ts = Series(
        1,
        date_range(
            "2024-04-19", "2024-04-20", tz="Africa/Cairo", freq="15min"
        ),
    )
...
def test_resample_dtype_preservation(unit):
    ...
    df = DataFrame(
        {
            "date": date_range(
                start="2016-01-01", periods=4, freq="W"
            ).as_unit(unit),
            "group": [1, 1, 2, 2],
...
def test_resample_dtype_coercion(unit):
    ...
    df = {"a": [1, 3, 1, 4]}
    df = DataFrame(
        df, index=date_range("2017-01-01", "2017-01-04").as_unit(unit)
    )
...
def test_monthly_resample_error(unit):
    ...
    ts = Series(
        np.random.default_rng(2).standard_normal(len(dates)), index=dates
    )
    # it works!
    ts.resample("ME")
...
def test_nanosecond_resample_error():
    ...
    exp_indx = date_range(
        start=pd.to_datetime(exp_start), periods=10, freq="100ns"
    )
...
def test_anchored_lowercase_buglet(unit):
    dates = date_range("4/16/2012 20:00", periods=50000, freq="s").as_unit(
        unit
    )
    ts = Series(
        np.random.default_rng(2).standard_normal(len(dates)), index=dates
...
def test_resample_group_info(unit):
    ...
    ix = date_range(
        start=ts.index.min(), end=ts.index.max(), freq="30min"
    ).as_unit(unit)
...
def test_resample_size(unit):
    ...
    ix = date_range(
        start=left.index.min(), end=ts.index.max(), freq="7min"
    ).as_unit(unit)
...
def test_resample_apply_product(duplicates, unit):
    ...
    expected = DataFrame(
        np.array(
            [[0, 24], [60, 210], [336, 720], [990, 1716]], dtype=np.int64
        ),
        index=DatetimeIndex(
...
def test_resample_calendar_day_with_dst(
    ...
    ts = Series(
        1.0,
        date_range(first, last, freq=freq_in, tz="Europe/Amsterdam").as_unit(
            unit
        ),
    )
    ...
    expected = Series(
        1.0,
        date_range(
            first, exp_last, freq=freq_out, tz="Europe/Amsterdam"
        ).as_unit(unit),
    )
    tm.assert_series_equal(result, expected)
...
def test_resample_unsigned_int(any_unsigned_int_numpy_dtype, unit):
    ...
    df = DataFrame(
        index=date_range(
            start="2000-01-01", end="2000-01-03 23", freq="12h"
        ).as_unit(unit),
        columns=["x"],
        data=[0, 1, 0] * 2,
    ...
    expected = DataFrame(
        ...
        columns=["x"],
        index=date_range(
            start="2000-01-01", end="2000-01-03 23", freq="D"
        ).as_unit(unit),
    )
    tm.assert_frame_equal(result, expected)
...
def test_long_rule_non_nano():
    ...
    ser = Series(
        [1, 4, 2, 8, 5, 7, 1, 4, 2, 8, 5, 7, 1, 4, 2, 8, 5], index=idx
    )
...
def test_resample_empty_series_with_tz():
    ...
    result = df.resample(
        "2MS", on="ts", closed="left", label="left", origin="start"
    )["values"].sum()
...
@pytest.mark.parametrize(
    "freq", ["2M", "2m", "2Q", "2Q-SEP", "2q-sep", "1Y", "2Y-MAR"]
)
