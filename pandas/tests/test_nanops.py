...
class TestnanopsDataFrame:
        ...
        **kwargs,
        ...
        for axis in list(range(targarval.ndim)) + [None]:
            ...
            if testfunc.__name__ in ["nanargmax", "nanargmin"] and (
                ...
                with pytest.raises(
                    ValueError, match="Encountered .* NA value"
                ):
    ...
    def check_funs(
        ...
        **kwargs,
        ...
        if allow_complex:
            ...
            self.check_fun(
                testfunc, targfunc, "arr_complex_nan", skipna, **kwargs
            )
            if allow_all_nan:
                self.check_fun(
                    testfunc, targfunc, "arr_nan_nanj", skipna, **kwargs
                )
        ...
        if allow_tdelta:
            try:
                ...
                self.check_fun(
                    testfunc, targfunc, "arr_tdelta", skipna, **kwargs
                )
    ...
    def test_nan_funcs(self, nan_op, np_op, skipna):
        self.check_funs(
            nan_op, np_op, skipna, allow_all_nan=False, allow_date=False
        )
...
class TestEnsureNumeric:
    ...
    def test_convertable_values(self):
        with pytest.raises(
            TypeError, match="Could not convert string '1' to numeric"
        ):
...
class TestNanvarFixedValues:
    ...
    def test_nanvar_ddof(self):
        ...
        tm.assert_almost_equal(
            variance_2, (n - 1.0) / (n - 2.0) * var, rtol=1e-2
        )
...
@pytest.mark.parametrize(
    "mask",
    [None, np.array([False, False, True]), np.array([True] + 9 * [False])],
)
@pytest.mark.parametrize(
    "min_count, expected_result", [(1, False), (101, True)]
)
...
@td.skip_if_32bit
@pytest.mark.parametrize(
    "min_count, expected_result", [(1, False), (2812191852, True)]
)
...
@pytest.mark.parametrize(
    "method", ["mean", "std", "var", "skew", "kurt", "min", "max"]
)
