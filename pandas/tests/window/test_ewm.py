...
def test_ewma_halflife_not_correct_type():
    ...
    with pytest.raises(ValueError, match=msg):
        Series(range(5)).ewm(
            halflife=1, times=np.arange(5).astype("datetime64[ns]")
        )
...
def test_ewma_with_times_equal_spacing(
    ...
    df = DataFrame({"A": data})
    result = df.ewm(
        halflife=halflife, min_periods=min_periods, times=times
    ).mean()
...
def test_ewm_with_nat_raises(halflife_with_times):
    ...
    with pytest.raises(
        ValueError, match="Cannot convert NaT values to integer"
    ):
...
def test_times_string_col_raises():
    ...
    df = DataFrame(
        {
            "A": np.arange(10.0),
            "time_col": date_range("2000", freq="D", periods=10),
        }
    )
...
def test_numeric_only_frame(arithmetic_win_operators, numeric_only):
    ...
    if op is not None:
        ...
        expected = (
            df[columns].agg([kernel]).reset_index(drop=True).astype(float)
        )
...
def test_numeric_only_series(arithmetic_win_operators, numeric_only, dtype):
    ...
    if numeric_only and dtype is object:
        msg = (
            f"ExponentialMovingWindow.{kernel} does not implement numeric_only"
        )
