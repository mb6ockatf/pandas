...
def test_rolling_apply_invalid_raw(bad_raw):
    with pytest.raises(
        ValueError, match="raw parameter must be `True` or `False`"
    ):
...
def test_center_reindex_frame(raw):
    # shifter index
    frame = DataFrame(
        range(100), index=date_range("2020-01-01", freq="D", periods=100)
    )
    s = [f"x{x:d}" for x in range(12)]
    minp = 10
    ...
    frame_rs = frame.rolling(window=25, min_periods=minp, center=True).apply(
        f, raw=raw
    )
