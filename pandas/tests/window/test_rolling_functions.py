...
def test_center_reindex_series(series, roll_func, kwargs, minp, fill_value):
    ...
    series_xp = (
        getattr(
            series.reindex(list(series.index) + s).rolling(
                window=25, min_periods=minp
            ),
            roll_func,
        )(**kwargs)
        .shift(-12)
...
def test_center_reindex_frame(frame, roll_func, kwargs, minp, fill_value):
    ...
    frame_xp = (
        getattr(
            frame.reindex(list(frame.index) + s).rolling(
                window=25, min_periods=minp
            ),
            roll_func,
        )(**kwargs)
...
def test_rolling_min_max_numeric_types(any_real_numpy_dtype):
    ...
    # correctness
    result = (
        DataFrame(np.arange(20, dtype=any_real_numpy_dtype))
        .rolling(window=5)
        .max()
    )
