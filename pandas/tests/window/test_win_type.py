...
def test_win_type_freq_return_none():
    # GH 48838
    freq_roll = Series(range(2), index=date_range("2020", periods=2)).rolling(
        "2s"
    )
...
def test_win_type_not_implemented():
    ...
    with pytest.raises(
        NotImplementedError, match="BaseIndexer subclasses not"
    ):
...
def test_cmov_window(step):
    ...
    result = (
        Series(vals)
        .rolling(5, win_type="boxcar", center=True, step=step)
        .mean()
    )
    expected_values = [
        np.nan,
...
def test_cmov_window_na_min_periods(step, min_periods):
    ...
    xp = vals.rolling(
        5, min_periods=min_periods, center=True, step=step
    ).mean()
    rs = vals.rolling(
...
def test_cmov_window_regular(win_types, step):
    ...
    rs = (
        Series(vals)
        .rolling(5, win_type=win_types, center=True, step=step)
        .mean()
    )
    tm.assert_series_equal(xp, rs)
...
def test_cmov_window_regular_missing_data(win_types, step):
    ...
    rs = (
        Series(vals)
        .rolling(5, win_type=win_types, min_periods=3, step=step)
        .mean()
    )
    tm.assert_series_equal(xp, rs)
