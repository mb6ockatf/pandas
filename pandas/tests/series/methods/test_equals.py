...
@pytest.mark.parametrize(
    "val",
    [1, 1.1, 1 + 1j, True, "abc", [1, 2], (1, 2), {1, 2}, {"a": 1}, None],
)
...
def test_equals_mismatched_nas(nulls_fixture, nulls_fixture2):
    ...
    elif (left is None and is_float(right)) or (
        right is None and is_float(left)
    ):
        assert ser.equals(ser2)
    else:
...
def test_equals_None_vs_float():
    # GH#44190
    left = Series(
        [-np.inf, np.nan, -1.0, 0.0, 1.0, 10 / 3, np.inf], dtype=object
    )
