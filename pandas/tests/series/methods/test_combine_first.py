...
class TestCombineFirst:
    def test_combine_first_period_datetime(self):
        ...
        pidx = period_range(
            start=Period("1950-1"), end=Period("1950-7"), freq="M"
        )
    ...
    def test_combine_first(self, using_infer_string):
        ...
        floats = Series(
            np.random.default_rng(2).standard_normal(20), index=index
        )
        strings = Series(
            [str(i) for i in range(10)], index=index[::2], dtype=object
        )
    ...
    def test_combine_first_dt64(self, unit):
        ...
        xp = Series(
            [datetime(2010, 1, 1), "2011"], dtype=f"datetime64[{unit}]"
        )
    ...
    def test_combine_mixed_timezone(self):
        ...
        multi_tz = Series(
            {
                pd.Timestamp(
                    "2019-05-01 01:00:00+0100", tz="Europe/London"
                ): 2.0,
