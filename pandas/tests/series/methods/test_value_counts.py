...
class TestSeriesValueCounts:
    def test_value_counts_datetime(self, unit):
        ...
        # normalize
        exp = Series(
            np.array([3.0, 2.0, 1]) / 6.0, index=exp_idx, name="proportion"
        )
    ...
    def test_value_counts_categorical(self):
        ...
        exp = Series(
            [3, 2, 1], name="count", index=Index(["c", "b", "a"], name="xxx")
        )
        tm.assert_series_equal(res, exp)
    ...
    def test_value_counts_categorical_with_nan(self):
        ...
        for ser in series:
            ...
            exp = Series(
                [2, 1], index=CategoricalIndex(["a", "b"]), name="count"
            )
            res = ser.value_counts(dropna=True)
            tm.assert_series_equal(res, exp)
            ...
            exp = Series(
                [3, 2, 1],
                index=CategoricalIndex([np.nan, "a", "b"]),
                name="count",
            )
            ...
            exp = Series(
                [2, 1, 3],
                index=CategoricalIndex(["a", "b", np.nan]),
                name="count",
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                Series(
                    [2, 1],
                    index=Index([True, False], dtype=object),
                    name="count",
                ),
            ),
            (
