...
def test_boolean_array_constructor():
    ...
    with pytest.raises(
        TypeError, match="values should be boolean numpy array"
    ):
...
def test_to_boolean_array():
    ...
    tm.assert_extension_array_equal(result, expected)
    result = pd.array(
        np.array([True, False, True], dtype=object), dtype="boolean"
    )
    tm.assert_extension_array_equal(result, expected)
    ...
    tm.assert_extension_array_equal(result, expected)
    result = pd.array(
        np.array([True, False, None], dtype=object), dtype="boolean"
    )
    tm.assert_extension_array_equal(result, expected)
...
def test_to_boolean_array_all_none():
    ...
    tm.assert_extension_array_equal(result, expected)
    result = pd.array(
        np.array([None, None, None], dtype=object), dtype="boolean"
    )
    tm.assert_extension_array_equal(result, expected)
...
def test_coerce_to_array():
    ...
    with pytest.raises(
        ValueError, match="values.shape and mask.shape must match"
    ):
...
def test_coerce_to_array_from_boolean_array():
    ...
    with pytest.raises(
        ValueError, match="cannot pass mask for BooleanArray input"
    ):
...
def test_to_boolean_array_from_strings():
    result = BooleanArray._from_sequence_of_strings(
        np.array(
            ["True", "False", "1", "1.0", "0", "0.0", np.nan], dtype=object
        ),
