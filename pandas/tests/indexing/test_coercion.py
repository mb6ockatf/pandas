...
class TestSetitemCoercion(CoercionBase):
    ...
    def test_setitem_index_object(self, val, exp_dtype):
        obj = pd.Series(
            [1, 2, 3, 4], index=pd.Index(list("abcd"), dtype=object)
        )
...
class TestWhereCoercion(CoercionBase):
    ...
    def test_where_float64(
        self, index_or_series, fill_val, exp_dtype, request
    ):
        klass = index_or_series
    ...
    def test_where_datetime64(self, index_or_series, fill_val, exp_dtype):
        ...
        obj = klass(
            pd.date_range("2011-01-01", periods=4, freq="D")._with_freq(None)
        )
    ...
    @pytest.mark.parametrize(
        "value",
        [pd.Timedelta(days=9), timedelta(days=9), np.timedelta64(9, "D")],
    )
...
class TestFillnaSeriesCoercion(CoercionBase):
    ...
    def test_fillna_series_period(self, index_or_series, fill_val):
        pi = pd.period_range("2016-01-01", periods=4, freq="D").insert(
            1, pd.NaT
        )
...
class TestReplaceSeriesCoercion(CoercionBase):
    ...
    def test_replace_series_datetime_tz(
        ...
        obj = pd.Series(
            self.rep[from_key], index=index, name="yyy"
        ).dt.as_unit("ns")
        assert obj.dtype == from_key
