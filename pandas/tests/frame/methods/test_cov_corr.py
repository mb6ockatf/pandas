...
class TestDataFrameCorr:
    ...
    def test_corr_scipy_method(self, float_frame, method):
        ...
        float_frame.loc[float_frame.index[:10], "A"] = float_frame["A"][
            10:20
        ].copy()
    ...
    def test_corr_nullable_integer(
        ...
        expected = DataFrame(
            np.ones((2, 2)), columns=["a", "b"], index=["a", "b"]
        )
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameCorrWith:
    ...
    def test_corrwith(self, datetime_frame, dtype):
        ...
        b = b.reindex(
            columns=b.columns[::-1], index=b.index[::-1][len(a) // 2 :]
        )
        del b["B"]
    ...
    def test_corrwith_index_intersection(self):
        ...
        df2 = DataFrame(
            np.random.default_rng(2).random(size=(10, 3)),
            columns=["a", "b", "c"],
        )
