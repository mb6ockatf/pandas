...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_date_fill_value(self):
        ...
        res = df.reindex(
            index=range(4), columns=["A", "B", "C"], fill_value=fv
        )
    ...
    def test_reindex_methods(self, method, expected_values):
        ...
        switched_method = (
            "pad"
            if method == "backfill"
            else "backfill" if method == "pad" else method
        )
        ...
        df = DataFrame(
            {
                ...
                "joline": np.random.default_rng(2).standard_normal(20).round(3)
                * 10,
            }
        )
        icol = ["jim", "joe", "jolie"]
    ...
    def test_reindex_with_categoricalindex(self):
        ...
        expected = DataFrame({"A": [0, 1], "B": Series(list("ab"))}).set_index(
            "B"
        )
        tm.assert_frame_equal(result, expected, check_index_type=True)
        ...
        expected = DataFrame(
            {
                "A": [0, np.nan],
                "B": Series(list("ae")).astype(
                    CategoricalDtype(cats, ordered=True)
                ),
            }
        ).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
