...
def left_w_dups(left_no_dup):
    return concat(
        [left_no_dup, DataFrame({"a": ["a"], "b": ["cow"]}, index=[3])],
        sort=True,
    )
...
class TestDataFrameJoin:
    def test_join(self, multiindex_dataframe_random_data):
        ...
        expected = DataFrame(
            expected, index=frame.index, columns=frame.columns
        )
    ...
    def test_frame_join_tzaware(self):
        ...
        test1 = DataFrame(
            np.zeros((6, 3)),
            index=date_range(
                "2012-11-15 00:00:00", periods=6, freq="100ms", tz=tz
            ),
        )
        test2 = DataFrame(
            np.zeros((3, 3)),
            index=date_range(
                "2012-11-15 00:00:00", periods=3, freq="250ms", tz=tz
            ),
            columns=range(3, 6),
        )
