...
class TestDataFrameCombineFirst:
    ...
    def test_combine_first_timezone(self, unit):
        # see gh-7630
        data1 = (
            pd.to_datetime("20100101 01:01").tz_localize("UTC").as_unit(unit)
        )
        df1 = DataFrame(
        ...
        data2 = (
            pd.to_datetime("20121212 12:12").tz_localize("UTC").as_unit(unit)
        )
        df2 = DataFrame(
    ...
    def test_combine_first_timezone4(self, unit):
        # different tz
        dts1 = pd.date_range(
            "2015-01-01", "2015-01-05", tz="US/Eastern", unit=unit
        )
        df1 = DataFrame({"DATE": dts1})
    ...
    def test_combine_first_timezone5(self, unit):
        dts1 = pd.date_range(
            "2015-01-01", "2015-01-02", tz="US/Eastern", unit=unit
        )
        df1 = DataFrame({"DATE": dts1})
    ...
    def test_combine_first_string_dtype_only_na(self, nullable_string_dtype):
        ...
        df2 = DataFrame(
            {"a": ["85"], "b": [pd.NA]}, dtype=nullable_string_dtype
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            pd.Interval(left=0, right=1),
            pd.Interval(left=2, right=3, closed="left"),
        ),
    ],
)
...
def test_combine_first_timestamp_bug_NaT():
    ...
    result = frame.combine_first(other)
    expected = DataFrame(
        [[pd.NaT, datetime(2020, 1, 1), datetime(2020, 1, 2)]],
        columns=["a", "b", "c"],
    )
