...
class TestResetIndex:
    def test_reset_index_empty_rangeindex(self):
        ...
        df = DataFrame(
            columns=["brand"],
            dtype=np.int64,
            index=RangeIndex(0, 0, 1, name="foo"),
        )
    ...
    def test_reset_index(self, float_frame):
        ...
        tm.assert_index_equal(
            deleveled.index, Index(range(len(deleveled))), exact=True
        )
        ...
        expected = DataFrame(
            {
                "level_0": Categorical.from_codes(
                    codes[0], categories=["A", "B"]
                ),
                "level_1": Categorical.from_codes(
                    codes[1], categories=["a", "b"]
                ),
                "col": range(4),
            }
        )
