...
class TestDataFrameConstructors:
    ...
    def test_datetimelike_values_with_object_dtype(
        ...
        if frame_or_series is DataFrame:
            ...
            assert obj._mgr.blocks[0].values.dtype == object
            assert isinstance(
                obj._mgr.blocks[0].values.ravel()[0], scalar_type
            )
    ...
    def test_constructor_mixed_dtypes(self, typ, ad):
        if typ == "int":
            ...
            arrays = [
                np.array(
                    np.random.default_rng(2).integers(10, size=10), dtype=d
                )
                for d in dtypes
            ]
    ...
    def test_constructor_dict_cast(self, using_infer_string):
        ...
        assert len(frame) == 3
        assert (
            frame["B"].dtype == np.object_ if not using_infer_string else "str"
        )
        assert frame["A"].dtype == np.float64
    ...
    def test_constructor_dict_cast2(self):
        ...
        test_data = {
            ...
            "B": dict(
                zip(range(15), np.random.default_rng(2).standard_normal(15))
            ),
        }
    ...
    def test_constructor_list_of_lists(self, using_infer_string):
        ...
        assert (
            df["str"].dtype == np.object_ if not using_infer_string else "str"
        )
    ...
    def test_constructor_list_of_series_aligned_index(self):
        series = [
            Series(i, index=["b", "a", "c"], name=str(i)) for i in range(3)
        ]
    ...
    def test_constructor_Series_copy_bug(self, float_frame):
        df = DataFrame(
            float_frame["A"], index=float_frame.index, columns=["A"]
        )
        df.copy()
    ...
    def test_constructor_mix_series_nonseries(self, float_frame):
        df = DataFrame(
            {"A": float_frame["A"], "B": list(float_frame["B"])},
            columns=["A", "B"],
        )
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame(
                {"A": float_frame["A"], "B": list(float_frame["B"])[:-2]}
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            PeriodArray(
                pd.period_range(start="1/1/2017", end="1/1/2018", freq="M")
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "cons",
        [Series, Index, DatetimeIndex, DataFrame, pd.array, pd.to_datetime],
    )
...
class TestDataFrameConstructorIndexInference:
    ...
    def test_frame_from_dict_with_mixed_tzaware_indexes(self):
        ...
        exp_index = Index(
            list(ser2.index) + list(ser3.index) + list(ser4.index),
            dtype=object,
        )
        ...
        exp_index3 = Index(
            list(ser2.index) + list(ser4.index) + list(ser3.index),
            dtype=object,
        )
        ...
        exp_index3 = Index(
            list(ser3.index) + list(ser2.index) + list(ser4.index),
            dtype=object,
        )
        ...
        exp_index4 = Index(
            list(ser4.index) + list(ser3.index) + list(ser2.index),
            dtype=object,
        )
    ...
    def test_dict_data_arrow_column_expansion(
        ...
        cols = pd.arrays.ArrowExtensionArray(
            pa.array(
                col_vals,
                type=pa.dictionary(pa.int8(), getattr(pa, col_type)()),
            )
        )
...
class TestDataFrameConstructorWithDatetimeTZ:
    ...
    def test_construction_from_ndarray_datetimelike(self):
        ...
        df = DataFrame(arr)
        assert all(
            isinstance(block.values, DatetimeArray) for block in df._mgr.blocks
        )
