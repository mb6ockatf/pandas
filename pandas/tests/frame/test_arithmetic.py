...
class TestFrameComparisons:
    ...
    def test_timestamp_compare(self, left, right):
        ...
        df = DataFrame(
            {
                ...
                "intcol": np.random.default_rng(2).integers(
                    1000000000, size=10
                ),
        ...
        df.loc[np.random.default_rng(2).random(len(df)) > 0.5, "dates2"] = (
            pd.NaT
        )
...
class TestFrameFlexComparisons:
    ...
    def test_df_flex_cmp_constant_return_types(self, comparison_op):
        ...
        result = getattr(df, comparison_op.__name__)(
            const
        ).dtypes.value_counts()
    ...
    def test_df_flex_cmp_constant_return_types_empty(self, comparison_op):
        ...
        empty = df.iloc[:0]
        result = getattr(empty, comparison_op.__name__)(
            const
        ).dtypes.value_counts()
        tm.assert_series_equal(
            result, Series([2], index=[np.dtype(bool)], name="count")
...
class TestFrameFlexArithmetic:
    ...
    def test_arith_flex_series_broadcasting(self, any_real_numpy_dtype):
        ...
        df = DataFrame(
            np.arange(3 * 2).reshape((3, 2)), dtype=any_real_numpy_dtype
        )
...
class TestFrameArithmetic:
    ...
    def test_operations_with_interval_categories_index(
        ...
        expected = DataFrame(
            [[getattr(n, op)(num) for n in data]], columns=ind
        )
        tm.assert_frame_equal(result, expected)
        ...
        if (op, dtype) in invalid:
            ...
            with pytest.raises(TypeError, match=msg):
                with tm.assert_produces_warning(
                    warn, match="evaluating in Python"
                ):
                    op(df, elem.value)
...
def test_frame_with_zero_len_series_corner_cases():
    ...
    # easy all-float case
    df = DataFrame(
        np.random.default_rng(2).standard_normal(6).reshape(3, 2),
        columns=["A", "B"],
    )
...
class TestFrameArithmeticUnsorted:
    ...
    def test_align_frame(self):
        ...
        half = ts[::2]
        result = ts + half.take(
            np.random.default_rng(2).permutation(len(half))
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_combine_series(
        ...
        _check_mixed_int(
            added,
            dtype={"A": "int64", "B": "float64", "C": "int64", "D": "int64"},
        )
        added = mixed_int_frame + (100 * series).astype("int32")
        _check_mixed_int(
            added,
            dtype={"A": "int32", "B": "float64", "C": "int32", "D": "int64"},
        )
    ...
    @pytest.mark.parametrize(
        "val", [[1, 2], (1, 2), np.array([1, 2]), range(1, 3)]
    )
...
def test_arith_reindex_with_duplicates():
    ...
    result = df1 + df2
    expected = DataFrame(
        [[np.nan, 0, 0]], columns=["first", "second", "second"]
    )
    tm.assert_frame_equal(result, expected)


@pytest.mark.parametrize(
    "to_add", [[Series([1, 1])], [Series([1, 1]), Series([1, 1])]]
)
