...
class TestTimestampReplace:
    def test_replace_out_of_pydatetime_bounds(self):
        ...
        msg = (
            "Out of bounds timestamp: 99999-01-01 00:00:00 with frequency 'ns'"
        )
        ...
        assert (
            result._value
            == Timestamp(np.datetime64("99999-01-01", "ms"))._value
        )
    ...
    def test_replace_dst_border(self, unit):
        ...
        assert result == expected
        assert (
            result._creso == getattr(NpyDatetimeUnit, f"NPY_FR_{unit}").value
        )
