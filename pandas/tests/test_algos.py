...
class TestFactorize:
    ...
    def test_object_factorize(self, writable):
        data = np.array(
            ["a", "c", None, np.nan, "a", "b", NaT, "c"], dtype=object
        )
        data.setflags(write=writable)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                CategoricalIndex(
                    ["a", "b"], categories=["a", "b"], dtype="category"
                ),
            ),
            (
    ...
    def test_factorize_interval_non_nano(self, unit):
        # GH#56099
        left = DatetimeIndex(["2016-01-01", np.nan, "2015-10-11"]).as_unit(
            unit
        )
        right = DatetimeIndex(["2016-01-02", np.nan, "2015-10-15"]).as_unit(
            unit
        )
        ...
        assert (
            cats3.dtype == f"interval[datetime64[{unit}, US/Pacific], right]"
        )
...
class TestUnique:
    ...
    def test_categorical(self):
        ...
        # CI -> return CI
        ci = CategoricalIndex(
            Categorical(list("baabc"), categories=list("abc"))
        )
    ...
    def test_order_of_appearance_dt64(self, unit):
        ser = Series(
            [Timestamp("20160101"), Timestamp("20160101")]
        ).dt.as_unit(unit)
        result = pd.unique(ser)
        expected = np.array(
...
class TestIsin:
    ...
    @pytest.mark.parametrize(
        "dtype1", ["m8[ns]", "M8[ns]", "M8[ns, UTC]", "period[D]"]
    )
    ...
    @pytest.mark.parametrize(
        "dtype", ["m8[ns]", "M8[ns]", "M8[ns, UTC]", "period[D]"]
    )
...
class TestValueCounts:
    ...
    def test_value_counts_dtypes(self):
        ...
        with pytest.raises(TypeError, match=msg):
            algos.value_counts_internal(
                np.array(["1", 1], dtype=object), bins=1
            )
    ...
    def test_value_counts_dropna(self):
        ...
        tm.assert_series_equal(
            Series([True] * 3 + [False] * 2 + [None] * 5).value_counts(
                dropna=True
            ),
            Series(
                [3, 2], index=Index([True, False], dtype=object), name="count"
            ),
        )
        tm.assert_series_equal(
    ...
    def test_value_counts_normalized(self, dtype):
        ...
        expected = Series(
            [0.6, 0.4],
            index=Series([2.0, 1.0], dtype=dtype),
            name="proportion",
        )
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                np.array(
                    [(1 + 1j), 0j, (1 + 0j), 1j, (1 + 2j)], dtype=complex
                ),
            )
        ],
    )
...
class TestMode:
    ...
    def test_datelike_mode(self):
        ...
        ser = Series(
            ["2011-01-03", "2013-01-02", "1900-05-03"], dtype="M8[ns]"
        )
