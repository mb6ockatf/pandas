...
@pytest.mark.parametrize(
    "a,b", [(1.1, 1.11), (0.1, 0.101), (0.000011, 0.001012)]
)
...
@pytest.mark.parametrize(
    "a,b", [(0, 0), (0, 0.0), (0, np.float64(0)), (0.00000001, 0)]
)
...
def test_mismatched_na_assert_almost_equal(left, right):
    ...
    if left is right:
        ...
        with pytest.raises(
            AssertionError, match="DataFrame.iloc.* are different"
        ):
            tm.assert_frame_equal(
                DataFrame(left_arr, dtype=object),
                DataFrame(right_arr, dtype=object),
...
NESTED_CASES = [
    ...
    (
        np.array(
            [
                np.array(
                    [np.array([1, 2, 3]), np.array([4, 5])], dtype=object
                ),
                np.array(
                    [np.array([6]), np.array([7, 8]), np.array([9])],
                    dtype=object,
                ),
            ],
            dtype=object,
    ...
    (
        np.array(
            [
                np.array(
                    [{"f1": 1, "f2": np.array(["a", "b"], dtype=object)}],
                    dtype=object,
                ),
