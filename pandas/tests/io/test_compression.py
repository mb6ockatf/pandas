...
def test_series_compression_defaults_to_infer(
    ...
    with tm.ensure_clean("compressed" + extension) as path:
        ...
        if "squeeze" in read_kwargs:
            ...
            del kwargs["squeeze"]
            output = read_method(
                path, compression=compression_only, **kwargs
            ).squeeze("columns")
        else:
            output = read_method(
...
def test_compression_warning(compression_only):
    ...
    with tm.ensure_clean() as path:
        with icom.get_handle(
            path, "w", compression=compression_only
        ) as handles:
...
def test_ambiguous_archive_zip():
    with tm.ensure_clean(filename=".zip") as path:
        ...
        with pytest.raises(
            ValueError, match="Multiple files found in ZIP file"
        ):
            pd.read_csv(path)
...
def test_ambiguous_archive_tar(tmp_path):
    ...
    with pytest.raises(
        ValueError, match="Multiple files found in TAR archive"
    ):
        pd.read_csv(tarpath)
