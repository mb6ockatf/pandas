...
def test_append_series(setup_path):
    with ensure_clean_store(setup_path) as store:
        # basic
        ss = Series(
            range(20), dtype=np.float64, index=[f"i_{i}" for i in range(20)]
        )
        ts = Series(
        ...
        # multi-index
        mi = DataFrame(
            np.random.default_rng(2).standard_normal((5, 1)), columns=["A"]
        )
...
def test_append_with_strings(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        tm.assert_series_equal(
            store.select("ss4"), concat([df["B"], df2["B"]])
        )
...
def test_append_hierarchical(
    tmp_path, setup_path, multiindex_dataframe_random_data
):
...
def test_append_misc_empty_frame(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(
            np.random.default_rng(2).random((10, 3)), columns=list("ABC")
        )
        store.append("df", df)
        tm.assert_frame_equal(store.select("df"), df)
        store.append("df", df_empty)
