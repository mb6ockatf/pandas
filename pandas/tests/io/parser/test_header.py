...
def test_negative_multi_index_header(all_parsers, header):
    ...
    with pytest.raises(
        ValueError,
        match="cannot specify multi-index header with negative integers",
    ):
...
def test_read_csv_multiindex_columns(all_parsers):
    ...
    expected = DataFrame(
        [[0.86, 0.67, 0.88, 0.78, 0.81], [0.86, 0.67, 0.88, 0.78, 0.82]],
        columns=mi,
    )
...
def test_header_none_and_implicit_index_in_second_row(all_parsers):
    ...
    data = "x,1\ny,2,5\nz,3\n"
    with pytest.raises(
        ParserError, match="Expected 2 fields in line 2, saw 3"
    ):
