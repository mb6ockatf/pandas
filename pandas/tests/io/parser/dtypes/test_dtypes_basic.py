...
4,5.5"""
    ...
    with pytest.raises(
        TypeError, match="data type [\"']foo[\"'] not understood"
    ):
...
def test_numeric_dtype(all_parsers, any_real_numpy_dtype):
    ...
    result = parser.read_csv(
        StringIO(data), header=None, dtype=any_real_numpy_dtype
    )
...
@pytest.mark.parametrize(
    "float_precision", [None, "legacy", "high", "round_trip"]
)
...
"""
        ...
        expected = DataFrame(
            {
                "a": pd.array(
                    ["a", "b"], dtype=pd.StringDtype(string_storage)
                ),
                "b": pd.array(
                    ["x", pd.NA], dtype=pd.StringDtype(string_storage)
                ),
            },
        )
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "dtype", ["O", object, "object", np.object_, str, np.str_]
)
...
z,a"""
    ...
    expected_dtype = (
        pd.StringDtype(na_value=np.nan) if dtype is str else object
    )
    expected = DataFrame(
        {
    ...
    expected = DataFrame(
        {
            "a": pd.Series(["x", "y", "z"], dtype=expected_dtype),
            "b": pd.Series(
                ["a", "a", "a"], dtype=pd.StringDtype(na_value=np.nan)
            ),
        },
        columns=pd.Index(["a", "b"], dtype=pd.StringDtype(na_value=np.nan)),
    )
...
NVDA,20230301181139587,2023552585717889827,2023552585717263361"""
    orders = all_parsers.read_csv(
        StringIO(data), dtype={"ID_DEAL": pd.Int64Dtype()}
    )
    assert (
        len(orders.loc[orders["ID_DEAL"] == 2023552585717263358, "ID_DEAL"])
        == 1
    )
    assert (
        len(orders.loc[orders["ID_DEAL"] == 2023552585717263359, "ID_DEAL"])
        == 1
    )
    assert (
        len(orders.loc[orders["ID_DEAL"] == 2023552585717263360, "ID_DEAL"])
        == 2
    )
    assert (
        len(orders.loc[orders["ID_DEAL"] == 2023552585717263361, "ID_DEAL"])
        == 2
    )
...
4,5,6"""
    ...
    result = parser.read_csv(
        StringIO(data), usecols=["a", "c"], dtype={"a": object}
    )
    if parser.engine == "pyarrow":
        values = [1, 4]
    else:
