...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            functools.partial(pd.to_pickle, protocol=4), id="pandas_proto_4"
        ),
        pytest.param(
...
class TestCompression:
    ...
    def test_write_explicit_bad(self, compression, get_random_path):
        ...
        with tm.ensure_clean(get_random_path) as path:
            with pytest.raises(
                ValueError, match="Unrecognized compression type"
            ):
...
def test_pickle_binary_object_compression(compression):
    ...
    assert buffer.getvalue() == reference or compression in (
        "gzip",
        "zip",
        "tar",
    )
...
@pytest.mark.parametrize(
    "protocol", [pickle.DEFAULT_PROTOCOL, pickle.HIGHEST_PROTOCOL]
)
