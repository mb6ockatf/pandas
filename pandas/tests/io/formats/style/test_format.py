...
def test_format_index_with_na_rep():
    ...
    ctx = df.style.format_index(None, na_rep="--", axis=1)._translate(
        True, True
    )
...
def test_format_escape_html(escape, exp):
    ...
    assert (
        styler._translate(True, True)["head"][0][1]["display_value"]
        == f"&{chars}&"
    )
...
def test_format_with_precision(precision, expected):
    ...
    for col, exp in enumerate(expected):
        ...
        assert (
            ctx["head"][0][col + 1]["display_value"] == exp
        )  # format_index test
...
def test_format_options():
    df = DataFrame(
        {"int": [2000, 1], "float": [1.009, None], "str": ["&<", "&~"]}
    )
    ...
    with option_context("styler.format.escape", "latex"):
        ctx_with_op = df.style._translate(True, True)
        assert (
            ctx_with_op["body"][1][3]["display_value"]
            == "\\&\\textasciitilde "
        )
...
def test_relabel_index(styler_multi):
    ...
    assert {"value": "X", "display_value": 1}.items() <= ctx["body"][0][
        0
    ].items()
    assert {"value": "y", "display_value": 2}.items() <= ctx["body"][0][
        1
    ].items()
    assert {"value": "Y", "display_value": 3}.items() <= ctx["body"][1][
        0
    ].items()
    assert {"value": "x", "display_value": 4}.items() <= ctx["body"][1][
        1
    ].items()
...
def test_relabel_columns(styler_multi):
    ...
    ctx = styler_multi._translate(True, True)
    assert {"value": "A", "display_value": 1}.items() <= ctx["head"][0][
        3
    ].items()
    assert {"value": "B", "display_value": 3}.items() <= ctx["head"][0][
        4
    ].items()
    assert {"value": "b", "display_value": 2}.items() <= ctx["head"][1][
        3
    ].items()
    assert {"value": "a", "display_value": 4}.items() <= ctx["head"][1][
        4
    ].items()
...
def test_relabel_roundtrip(styler):
    ...
    ctx = styler._translate(True, True)
    assert {"value": "x", "display_value": "x"}.items() <= ctx["body"][0][
        0
    ].items()
    assert {"value": "y", "display_value": "y"}.items() <= ctx["body"][1][
        0
    ].items()
