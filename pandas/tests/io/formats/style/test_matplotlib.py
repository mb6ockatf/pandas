...
def test_background_gradient_gmap_array(styler_blank, axis, gmap, expected):
    ...
    result = (
        styler_blank.background_gradient(axis=axis, gmap=gmap)._compute().ctx
    )
...
@pytest.mark.parametrize(
    "gmap, axis",
    [([1, 2, 3], 0), ([1, 2], 1), (np.array([[1, 2], [1, 2]]), None)],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            Series([2, 1], index=["Y", "X"]),
            0,
            [[1, 1], [2, 2]],
        ),  # reverse the index
        (
            Series([2, 1], index=["B", "A"]),
            1,
            [[1, 2], [1, 2]],
        ),  # reverse the cols
        (
...
def test_background_gradient_gmap_wrong_dataframe(styler_blank, axis):
    ...
    msg = (
        "'gmap' is a DataFrame but underlying data for operations is a Series"
    )
    with pytest.raises(ValueError, match=msg):
...
def test_background_gradient_gmap_wrong_series(styler_blank):
    ...
    msg = (
        "'gmap' is a Series but underlying data for operations is a DataFrame"
    )
