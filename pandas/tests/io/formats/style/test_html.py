...
def test_html_template_extends_options():
    ...
    with open(
        "pandas/io/formats/templates/html.tpl", encoding="utf-8"
    ) as file:
        result = file.read()
...
def test_w3_html_format(styler):
    ...
    ).map(lambda x: "att1:v1;").set_table_attributes(
...
def test_colspan_w3():
    ...
    assert (
        '<th class="col_heading level0 col0" colspan="2">l0</th>'
        in styler.to_html()
    )
...
def test_rowspan_w3():
    ...
    styler = Styler(df, uuid="_", cell_ids=False)
    assert (
        '<th class="row_heading level0 row0" rowspan="2">l0</th>'
        in styler.to_html()
    )
...
def test_sticky_basic(styler, index, columns, index_name):
    ...
    assert (
        left_css.format("thead tr th:nth-child(1)", "3 !important") in res
    ) is index
    ...
    assert (
        top_css.format(
            "thead tr:nth-child(1) th", "0", "2", "  height: 25px;\n"
        )
        in res
    ) is (columns and index_name)
    assert (
        top_css.format(
            "thead tr:nth-child(2) th", "25", "2", "  height: 25px;\n"
        )
        in res
    ) is (columns and index_name)
    assert (
        top_css.format("thead tr:nth-child(1) th", "0", "2", "") in res
    ) is (columns and not index_name)
...
def test_sticky_mi(styler_mi, index, columns):
    ...
    assert (
        left_css.format("thead tr th:nth-child(2)", "75", "3 !important")
        in res
    ) is index
    ...
    assert (
        top_css.format("thead tr:nth-child(2) th", "25", "2") in res
    ) is columns
...
def test_sticky_levels(styler_mi, index, columns, levels):
    styler_mi.index.names, styler_mi.columns.names = ["zero", "one"], [
        "zero",
        "one",
    ]
    if index:
    ...
    assert (
        top_css.format("thead tr:nth-child(2) th", "0", "2") in res
    ) is columns
...
def test_sparse_options(sparse_index, sparse_columns):
    ...
    default_html = (
        styler.to_html()
    )  # defaults under pd.options to (True , True)
    ...
    html2 = styler.to_html(
        sparse_index=sparse_index, sparse_columns=sparse_columns
    )
    assert html1 == html2
...
def test_maximums(styler_mi, rows, cols):
    ...
    assert (
        ">2</td>" in result
    ) is not cols  # first trimmed horizontal element
...
def test_hiding_index_columns_multiindex_trimming():
    ...
    with option_context(
        "styler.render.max_rows", 4, "styler.render.max_columns", 4
    ):
        result = styler.to_html()
...
def test_rendered_links(type, text, exp, found):
    ...
    assert (
        text in result
    ) is not exp  # test conversion done when expected and not
