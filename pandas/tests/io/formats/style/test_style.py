...
def test_render_trimming_mi():
    ...
    assert {"class": "data row_trim col_trim"}.items() <= ctx["body"][2][
        4
    ].items()
...
def test_copy(comprehensive, render, deepcopy, mi_styler, mi_styler_comp):
    ...
    s2 = (
        copy.deepcopy(styler) if deepcopy else copy.copy(styler)
    )  # make copy and check
    ...
    if (
        not deepcopy
    ):  # check memory locations are equal for all included attributes
        for attr in [
            a for a in styler.__dict__ if (not callable(a) and a not in excl)
        ]:
...
def test_export(mi_styler_comp, mi_styler):
    ...
    for attr in exp_attrs:
        ...
        assert not (
            all(check)
            if (hasattr(check, "__iter__") and len(check) > 0)
            else check
        )
...
def test_hide_raises(mi_styler):
    ...
    with pytest.raises(ValueError, match=msg):
        mi_styler.hide(
            axis="index", subset="something", level="something else"
        )
...
def test_hide_index_level(mi_styler, level):
    mi_styler.index.names, mi_styler.columns.names = ["zero", "one"], [
        "zero",
        "one",
    ]
...
def test_apply_map_header(method, axis):
    ...
    func = {
        "apply": lambda s: [
            "attr: val" if ("A" in v or "C" in v) else "" for v in s
        ],
...
def test_apply_map_header_raises(mi_styler):
    # GH 41893
    with pytest.raises(
        ValueError, match="No axis named bad for object type DataFrame"
    ):
...
class TestStyler:
    ...
    def test_apply_dataframe_return(self, index, columns):
        ...
        assert (
            result[(0, 1)] == [("color", "red")]
        ) is index  # (X,Y) only if index
        assert (
            result[(1, 0)] == [("color", "red")]
    ...
    def test_apply_subset(self, slice_, axis, df):
        ...
        result = (
            df.style.apply(h, axis=axis, subset=slice_, color="baz")
            ._compute()
            .ctx
        )
        expected = {
    ...
    def test_map_subset(self, slice_, df):
        result = (
            df.style.map(lambda x: "color:baz;", subset=slice_)._compute().ctx
        )
        expected = {
            (r, c): [("color", "baz")]
            for r, row in enumerate(df.index)
    ...
    def test_maybe_convert_css_to_tuples(self):
        ...
        assert (
            maybe_convert_css_to_tuples("a:b;c: url('data:123');") == expected
        )
    ...
    def test_bad_apply_shape(self):
        ...
        with pytest.raises(ValueError, match=msg.format("index")):
            df.style._apply(
                lambda x: Series(["a:v;", ""], index=["A", "C"]), axis=0
            )
        ...
        with pytest.raises(ValueError, match=msg.format("columns")):
            df.style._apply(
                lambda x: Series(["a:v;", ""], index=["X", "Z"]), axis=1
            )
    ...
    def test_hide_single_index(self, df):
        ...
        ctx4 = (
            df.set_index("A").style.hide(axis="index")._translate(True, True)
        )
    ...
    def test_set_data_classes(self, classes):
        ...
        assert (
            '<td id="T__row0_col1" class="data row0 col1 test-class" >1</td>'
            in s
        )
...
def test_row_trimming_hide_index_mi():
    ...
    with option_context("styler.render.max_rows", 2):
        ctx = df.style.hide([(0, 0), (0, 1)], axis="index")._translate(
            True, True
        )
    assert len(ctx["body"]) == 3
    ...
    assert {"value": "...", "is_visible": True}.items() <= ctx["body"][2][
        0
    ].items()

    for r, val in enumerate(["2", "3", "..."]):
        assert (
            ctx["body"][r][1]["display_value"] == val
