...
def test_parse_latex_table_styles(styler):
    ...
    assert (
        _parse_latex_table_styles(styler.table_styles, "label") == "{fig:item}"
    )
...
def test_parse_latex_table_wrapping(styler):
    ...
    assert (
        _parse_latex_table_wrapping(styler.table_styles, styler.caption)
        is False
    )
    assert (
        _parse_latex_table_wrapping(styler.table_styles, "some caption")
        is True
    )
    styler.set_table_styles(
        [
...
@pytest.mark.parametrize(
    "label, lab_exp", [(None, ""), ("tab:A", " \\label{tab:A}")]
)
...
def test_concat_recursion():
    ...
    styler2 = (
        DataFrame([[9], [2]]).style.hide([0]).highlight_min(color="green")
    )
