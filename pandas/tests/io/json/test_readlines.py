...
def test_read_datetime(request, engine):
    ...
    if engine == "pyarrow":
        ...
        request.applymarker(
            pytest.mark.xfail(reason=reason, raises=ValueError)
        )
    ...
    expected = DataFrame(
        [
            [1, "2020-03-05", "hector"],
            [2, "2020-04-08T09:58:49+00:00", "hector"],
        ],
        columns=["accounts", "date", "name"],
    )
    tm.assert_frame_equal(result, expected)
...
def test_readjson_unicode(request, monkeypatch, engine):
    ...
    with tm.ensure_clean("test.json") as path:
        monkeypatch.setattr(
            "locale.getpreferredencoding", lambda do_setlocale: "cp949"
        )
...
def test_readjson_lines_chunks_fileurl(request, datapath, engine):
    ...
    with read_json(
        file_url, lines=True, chunksize=1, engine=engine
    ) as url_reader:
