...
class TestUltraJSONTests:
    @pytest.mark.skipif(
        not IS64, reason="not compliant on 32-bit, xref #15865"
    )
        ...
        for new_locale in ("it_IT.UTF-8", "Italian_Italy"):
            if tm.can_set_locale(new_locale, lc_category):
                with tm.set_locale(new_locale, lc_category):
                    assert (
                        ujson.ujson_loads(ujson.ujson_dumps(4.78e60))
                        == 4.78e60
                    )
                    assert (
                        ujson.ujson_loads("4.78", precise_float=True) == 4.78
                    )
                break
    ...
    def test_encode_recursion_max(self):
        ...
        with pytest.raises(
            OverflowError, match="Maximum recursion level reached"
        ):
    ...
    def test_decode_depth_too_big(self, too_big_char):
        with pytest.raises(
            ValueError, match="Reached object decoding depth limit"
        ):
    ...
    @pytest.mark.parametrize(
        "long_input", [9223372036854775807, 18446744073709551615]
    )
    ...
    def test_default_handler(self):
        ...
        with pytest.raises(TypeError, match="I raise for anything"):
            ujson.ujson_dumps(
                _TestObject("foo"), default_handler=my_handler_raises
            )
        ...
        assert ujson.ujson_loads(
            ...
            ujson.ujson_dumps(
                _TestObject("foo"), default_handler=my_obj_handler
            )
        )

        obj_list = [_TestObject("foo"), _TestObject("bar")]
        assert json.loads(
            json.dumps(obj_list, default=str)
        ) == ujson.ujson_loads(
...
class TestNumpyJSONTests:
    ...
    def test_bool_array(self):
        ...
        output = np.array(
            ujson.ujson_loads(ujson.ujson_dumps(bool_array)), dtype=bool
        )
    ...
    def test_int_array(self, any_int_numpy_dtype):
        ...
        arr_output = np.array(
            ujson.ujson_loads(ujson.ujson_dumps(arr_input)),
    ...
    def test_float_max(self, float_numpy_dtype):
        ...
        tm.assert_almost_equal(
            klass(
                ujson.ujson_loads(ujson.ujson_dumps(num, double_precision=15))
            ),
            num,
        )
    ...
    def test_array_list(self):
        ...
        result = np.array(
            ujson.ujson_loads(ujson.ujson_dumps(arr)), dtype=object
        )
    ...
    def test_array_float(self):
        ...
        arr_out = np.array(
            ujson.ujson_loads(ujson.ujson_dumps(arr)), dtype=dtype
        )
...
class TestPandasJSONTests:
    ...
    def test_index(self):
        ...
        dec = _clean_dict(
            ujson.ujson_loads(ujson.ujson_dumps(i, orient="split"))
        )
        output = Index(**dec)
        ...
        output = Index(
            ujson.ujson_loads(ujson.ujson_dumps(i, orient="values")),
            name="index",
        )
        tm.assert_index_equal(i, output)

        output = Index(
            ujson.ujson_loads(ujson.ujson_dumps(i, orient="records")),
            name="index",
        )
        tm.assert_index_equal(i, output)

        output = Index(
            ujson.ujson_loads(ujson.ujson_dumps(i, orient="index")),
            name="index",
        )
        tm.assert_index_equal(i, output)
