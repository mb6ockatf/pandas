...
def test_dtype_single_str(parser):
    df_result = read_xml(
        StringIO(xml_types), dtype={"degrees": "str"}, parser=parser
    )
...
def test_dtype_nullable_int(parser):
    df_result = read_xml(
        StringIO(xml_types), dtype={"sides": "Int64"}, parser=parser
    )
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
...
def test_dtype_float(parser):
    df_result = read_xml(
        StringIO(xml_types), dtype={"degrees": "float"}, parser=parser
    )
    df_iter = read_xml_iterparse(
        xml_types,
        parser=parser,
...
def test_wrong_dtype(xml_books, parser, iterparse):
    with pytest.raises(
        ValueError,
        match=('Unable to parse string "Everyday Italian" at position 0'),
    ):
        read_xml(
            xml_books,
            dtype={"title": "Int64"},
...
def test_both_dtype_converters(parser):
    ...
    with tm.assert_produces_warning(
        ParserWarning, match="Both a converter and dtype"
    ):
        df_result = read_xml(
            StringIO(xml_types),
            dtype={"degrees": "str"},
...
def test_wrong_converters_type(xml_books, parser, iterparse):
    with pytest.raises(
        TypeError, match=("Type converters must be a dict or subclass")
    ):
        read_xml(
            xml_books,
            converters={"year", str},
...
def test_parse_dates_column_name(parser):
    df_result = read_xml(
        StringIO(xml_dates), parse_dates=["date"], parser=parser
    )
    df_iter = read_xml_iterparse(
        xml_dates,
        parser=parser,
...
<data>
  ...
  <row>
    ...
    with tm.assert_produces_warning(
        ...
        df_result = read_xml(
            StringIO(xml), parse_dates=["date"], parser=parser
        )
        df_iter = read_xml_iterparse(
            xml,
            parse_dates=["date"],
...
def test_wrong_parse_dates_type(xml_books, parser, iterparse):
    with pytest.raises(
        ...
        read_xml(
            xml_books, parse_dates={"date"}, parser=parser, iterparse=iterparse
        )
