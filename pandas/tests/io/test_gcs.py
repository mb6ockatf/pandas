...
@pytest.mark.parametrize(
    "format", ["csv", "json", "parquet", "excel", "markdown"]
)
    ...
    if format == "csv":
        ...
        class MockFileSystem(pa_fs.FileSystem):
            ...
            def from_uri(path):
                ...
                to_local = (
                    pathlib.Path(path.replace("gs://", "")).absolute().as_uri()
                )
...
def test_to_csv_compression_encoding_gcs(
    ...
    read_df = read_csv(
        path_gcs, index_col=0, compression="infer", encoding=encoding
    )
    tm.assert_frame_equal(df, read_df)
...
def test_to_parquet_gcs_new_file(monkeypatch, tmpdir):
    ...
    class MockGCSFileSystem(AbstractFileSystem):
        def open(self, path, mode="r", *args):
            ...
            return open(
                os.path.join(tmpdir, "test.parquet"), mode, encoding="utf-8"
            )
