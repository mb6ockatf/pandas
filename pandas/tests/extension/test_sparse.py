...
def data_for_compare(request):
    return SparseArray(
        [0, 0, np.nan, -2, -1, 4, 2, 3, 0, 0], fill_value=request.param
    )
...
class TestSparseArray(base.ExtensionTests):
    ...
    def test_reduce_series_numeric(
        ...
        super().test_reduce_series_numeric(
    ...
    def test_searchsorted(
        self, performance_warning, data_for_sorting, as_series
    ):
        with tm.assert_produces_warning(
            performance_warning, check_stacklevel=False
        ):
    ...
    def _compare_other(
        ...
        if isinstance(other, pd.Series):
            fill_value = op(
                data_for_compare.fill_value, other._values.fill_value
            )
            expected = SparseArray(
    ...
    def test_array(
        ...
        if (
            data_for_compare.dtype.fill_value == 0
            and comparison_op.__name__
            in [
                "eq",
                "ge",
                "le",
