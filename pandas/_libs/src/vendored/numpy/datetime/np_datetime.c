 ...
 */
int
cmp_npy_datetimestruct (const npy_datetimestruct *a,
 ...
 */
PyObject *
extract_utc_offset (PyObject *obj)
{
...
static inline int
scaleYearToEpoch (int64_t year, int64_t *result)
{
  return checked_int64_sub (year, 1970, result);
}

static inline int
scaleYearsToMonths (int64_t years, int64_t *result)
{
  return checked_int64_mul (years, 12, result);
}

static inline int
scaleDaysToWeeks (int64_t days, int64_t *result)
{
  if (days >= 0)
    {
      *result = days / 7;
...
static inline int
scaleDaysToHours (int64_t days, int64_t *result)
{
  return checked_int64_mul (days, 24, result);
}

static inline int
scaleHoursToMinutes (int64_t hours, int64_t *result)
{
  return checked_int64_mul (hours, 60, result);
}

static inline int
scaleMinutesToSeconds (int64_t minutes, int64_t *result)
{
  return checked_int64_mul (minutes, 60, result);
}

static inline int
scaleSecondsToMilliseconds (int64_t seconds, int64_t *result)
{
  return checked_int64_mul (seconds, 1000, result);
}

static inline int
scaleSecondsToMicroseconds (int64_t seconds, int64_t *result)
{
  return checked_int64_mul (seconds, 1000000, result);
}

static inline int
scaleMicrosecondsToNanoseconds (int64_t microseconds, int64_t *result)
{
  return checked_int64_mul (microseconds, 1000, result);
}

static inline int
scaleMicrosecondsToPicoseconds (int64_t microseconds, int64_t *result)
{
  return checked_int64_mul (microseconds, 1000000, result);
}

static inline int64_t
scalePicosecondsToFemtoseconds (int64_t picoseconds, int64_t *result)
{
  return checked_int64_mul (picoseconds, 1000, result);
}

static inline int64_t
scalePicosecondsToAttoseconds (int64_t picoseconds, int64_t *result)
{
				...
				const npy_datetimestruct *dts)
{
  ...
  if (base == NPY_FR_ms)
    {
      ...
      PD_CHECK_OVERFLOW (checked_int64_add
			 (milliseconds, dts->us / 1000, &milliseconds));
  ...
  PD_CHECK_OVERFLOW (checked_int64_add
		     (microseconds, dts->us, &microseconds));
  ...
  if (base == NPY_FR_ns)
      ...
      if (microseconds == min_nanoseconds / 1000 - 1)
	{
	  ...
	  PD_CHECK_OVERFLOW (checked_int64_add
			     (min_nanoseconds,
			      (dts->ps - _NS_MIN_DTS.ps) / 1000,
			      &nanoseconds));
	}
      else
	{
	  PD_CHECK_OVERFLOW (scaleMicrosecondsToNanoseconds
			     (microseconds, &nanoseconds));
	  PD_CHECK_OVERFLOW (checked_int64_add
			     (nanoseconds, dts->ps / 1000, &nanoseconds));
	}
  ...
  int64_t picoseconds;
  PD_CHECK_OVERFLOW (scaleMicrosecondsToPicoseconds
  ...
  if (base == NPY_FR_fs)
    {
      ...
      PD_CHECK_OVERFLOW (scalePicosecondsToFemtoseconds
			 (picoseconds, &femtoseconds));
      PD_CHECK_OVERFLOW (checked_int64_add
			 (femtoseconds, dts->as / 1000, &femtoseconds));
      return femtoseconds;
    }
  ...
  if (base == NPY_FR_as)
    {
      int64_t attoseconds;
      PD_CHECK_OVERFLOW (scalePicosecondsToAttoseconds
			 (picoseconds, &attoseconds));
      PD_CHECK_OVERFLOW (checked_int64_add
			 (attoseconds, dts->as, &attoseconds));
      return attoseconds;
    }
 ...
 */
npy_int64
extract_unit (npy_datetime *d, npy_datetime unit)
{
  assert (unit > 0);
 ...
 */
void
pandas_datetime_to_datetimestruct (npy_datetime dt, NPY_DATETIMEUNIT base,
				   npy_datetimestruct *out)
{
  ...
  switch (base)
    ...
    case NPY_FR_ns:
      ...
      set_datetimestruct_days (extract_unit (&dt, perday), out);
      out->hour =
	(npy_int32) extract_unit (&dt, 1000LL * 1000 * 1000 * 60 * 60);
	...
	(npy_int32) extract_unit (&dt, 1000LL * 1000 * 1000 * 1000 * 60 * 60);
      out->min =
	(npy_int32) extract_unit (&dt, 1000LL * 1000 * 1000 * 1000 * 60);
      ...
      if (out->hour < 0)
	{
      ...
      out->min =
	(npy_int32) extract_unit (&dt,
				  1000LL * 1000 * 1000 * 1000 * 1000 * 60);
      out->sec =
	(npy_int32) extract_unit (&dt, 1000LL * 1000 * 1000 * 1000 * 1000);
 ...
 */
void
pandas_timedelta_to_timedeltastruct (npy_timedelta td,
...
#endif
  return ((PyArray_DatetimeDTypeMetaData *) PyDataType_C_METADATA (dtype))->
    meta;
}
