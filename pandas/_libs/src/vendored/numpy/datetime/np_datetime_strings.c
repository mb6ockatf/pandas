...
parse_iso_8601_datetime (const char *str, int len, int want_exc,
			 ...
			 int format_len, FormatRequirement format_requirement)
{
  ...
  if (!isdigit (*substr))
      ...
      if (i == valid_ymd_sep_len)
	{
      ...
      comparison =
	compare_format (&format, &format_len, &ymd_sep, 1,
			format_requirement);
      if (comparison == COMPARISON_ERROR)
	{
	  goto parse_error;
 ...
 */
int
make_iso_8601_datetime (npy_datetimestruct *dts, char *outstr, size_t outlen,
...
string_too_short:
  PyErr_Format (PyExc_RuntimeError,
		...
		"was too short, with length %d", outlen);
  return -1;
}

int
make_iso_8601_timedelta (pandas_timedeltastruct *tds, char *outstr,
			 size_t *outlen)
{
  *outlen = 0;
  *outlen += snprintf (outstr, 60,	// NOLINT
		       "P%" NPY_INT64_FMT "DT%" NPY_INT32_FMT "H%"
		       NPY_INT32_FMT "M%" NPY_INT32_FMT, tds->days, tds->hrs,
		       tds->min, tds->sec);
  outstr += *outlen;
  ...
  if (tds->ns != 0)
  ...
  else if (tds->us != 0)
    {
      *outlen += snprintf (outstr, 9,	// NOLINT
			   ".%03" NPY_INT32_FMT "%03" NPY_INT32_FMT "S",
			   tds->ms, tds->us);
    }
  else if (tds->ms != 0)
    {
