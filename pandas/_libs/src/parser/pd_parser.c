...
static int
to_double (char *item, double *p_value, char sci, char decimal,
	   int *maybe_int)
{
  char *p_end = NULL;
  int error = 0;
...
static int
floatify (PyObject *str, double *result, int *maybe_int)
{
  char *data;
  PyObject *tmp = NULL;
  const char sci = 'E';
...
static void
pandas_parser_destructor (PyObject *op)
{
...
static int
pandas_parser_exec (PyObject *Py_UNUSED (module))
{
