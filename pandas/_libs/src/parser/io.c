...
char *
buffer_rd_bytes (void *source, size_t nbytes, size_t *bytes_read,
		 int *status, const char *encoding_errors)
{
  ...
  if (result == NULL)
    ...
    {
      PyObject *tmp =
	PyUnicode_AsEncodedString (result, "utf-8", encoding_errors);
      Py_DECREF (result);
      if (tmp == NULL)
	{
	  PyGILState_Release (state);
